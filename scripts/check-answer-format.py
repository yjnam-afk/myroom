...
JUDGE_MAX = 30  # | 구분 | 점검 항목 | 판정·조치 |, | 취약점 | 내용 | 대응 방안 | 등
...
def check_tables(a):
    ...
    for t in tables:
        ...
        if len(header) != 3:
            continue
        # 3열이 '설명'이면 나열표(2열 이름은 키워드·기술요소·단계 등 자유),
        # '판정·조치'류면 판정표. 그 밖은 비교표이므로 길이 제한 대상이 아니다.
        if header[2] == "설명":
            cap = LIST_MAX
        elif header[2] in ("판정·조치", "대응 방안", "조치", "판정"):
            cap = JUDGE_MAX
        else:
            continue
        for r in rows[2:]:
