...
export const EXTRAS: Record<string, SubnoteExtra> = {
  "chaos-test": { image: "/concept/book/chaos-test.png", easy: "카오스 테스트는 시스템 신뢰성을 확인하기 위해 실 서비스에 인위적 혼돈(Chaos)을 주입(Failure Injection)해, 출시 전 테스트에서 드러나지 않은 아키텍처 문제를 찾아내는 테스트입니다. 절차는 [정가실결문] — 정상 상태(CPU load·NW I/O 등 정량 지표 측정) → 가설 수립('DB가 다운돼도 정상 유지된다' 같은 시나리오) → 실험 디자인(작업 범위 최소화·롤백 계획 수립) → 결과 확인(장애 감지·전파·복구 시간을 정상 지표와 비교해 가설 검증) → 문제점 수정(지속 개선). 효율적 수행 방안은 Chaos Engineering Team 구축, Chaos Monkey·Kube Monkey·GameDay 같은 도구 활용, 개발팀→애플리케이션→스위칭→인프라 계층별 테스트입니다. '실 서비스에 일부러 장애를 낸다'는 발상과 롤백 계획 같은 안전장치가 짝으로 출제됩니다." },
  "refactoring": { image: "/concept/book/refactoring.png", images: ["/concept/extra/refactoring-process.png"], imagesLabel: "절차", easy: "리팩토링은 모듈의 외부 기능은 그대로 두고 내부 구조·관계만 단순화해 유지보수성을 높이는 기법입니다. 대상(코드 악취) [중긴큰긴산] — 중복된 코드, 긴 메소드, 큰 클래스, 긴 파라미터 리스트, 산탄총 수술(한 변경이 여러 클래스 수정 유발). 기법은 결합도 측면(클래스 이동 등), 응집도 측면(메소드 추출 등), 단순화, 은닉으로 분류됩니다. '기능 변경 없음'이 정의의 생명이고, 테스트(회귀)와 세트로 진행해야 안전하다는 것까지 쓰면 완성입니다." },
  "keyword-driven-testing": { image: "/concept/book/keyword-driven-testing.png", easy: "키워드 주도 테스트는 테스트를 '키워드(동작 단어)'로 조립하는 자동화 기법으로, ISO/IEC/IEEE 29119 Part 5에 명시된 국제 표준 테스트 기법입니다. 프레임워크 구성요소 — 테스트 라이브러리 저장소, 테스트 데이터 저장소, 편집기, 해석기와 데이터 시퀀서, 툴 브리지, 실행엔진, SUT(테스트 대상). 절차는 키워드 정의 → 키워드 테스트 케이스 작성 → 실행 → 키워드 리팩토링. 비개발자도 키워드 조합으로 테스트를 만들 수 있어 유지보수가 쉽다는 것이 강점이자 시험 포인트입니다." },
  "three-r": { image: "/concept/book/three-r.png", easy: "3R은 소프트웨어 생산성 극대화를 위해 레포지토리를 기반으로 역공학·재공학·재사용을 쓰는 공학적 접근법입니다. 역공학(Reverse): 코드·바이너리에서 설계 정보를 거꾸로 추출(코드→설계서). 재공학(Re-Engineering): 추출한 정보로 재설계·재구조화해 새 형태로 변환 — 유지보수성 향상. 재사용(Re-Use): 검증된 컴포넌트를 다시 사용 — 필요 속성(독립성·일반성)과 합성·생성 기법. '역공학은 이해, 재공학은 개선, 재사용은 활용'으로 역할을 구분하고, 셋의 중심에 레포지토리가 있다는 구조가 답안 뼈대입니다." },
  "turing-test": { image: "/concept/book/turing-test.png", easy: "튜링 테스트는 기계가 인간과 구별할 수 없을 정도의 지적 행동을 보이는지 대화로 확인하는 imitation game 방식의 테스트입니다. 판정자가 벽 너머의 사람과 기계에게 같은 질문을 던지고, 답만 보고 어느 쪽이 기계인지 구별하지 못하면 그 기계는 지능이 있다고 판정합니다. 활용은 이미지 인식(CAPTCHA — 사람만 풀 수 있는 문제로 봇을 거르는 역튜링 테스트)과 의료 분야 등. AI 시스템 테스트·ISO 29119-11과 묶어 'AI를 어떻게 검증하나' 계보로 출제됩니다." },
  "regression-test": { image: "/concept/book/regression-test.png", easy: "회귀 테스트는 프로그램을 수정·확장한 뒤 변경 부분뿐 아니라 기존 기능까지 다시 테스트해 오류를 검출하는 기법입니다. 고친 곳 때문에 멀쩡하던 곳이 망가지는 것을 잡는 게 목적입니다. 종류 3가지 — Reset All(전체 재수행: 확실하나 비용 큼), Selective(변경 영향 부분만 선별), Priority(우선순위 높은 것부터). 검출하는 오류도 2가지로 구분 — Ripple Effect(파급효과: 변경이 다른 모듈로 번짐)와 Side Effect(부작용: 의도치 않은 동작). 자동화(CI)와 묶어서 출제되기도 합니다." },
  "test-exit-criteria": { image: "/concept/book/test-exit-criteria.png", easy: "테스트 종료 조건은 추정 결함밀도, 커버리지 달성률, 일정·비용 같은 정량 지표로 하나의 테스트 레벨 또는 특정 목적 테스팅의 종료 시점을 정하는 완료 조건입니다. 완료 조건 [완목기커리스] — 테스트 완전성(모든 TC 수행·심각 결함 미존재), 테스트 목적(품질 목표 달성, 예: 만족도 90% 이상), 테스트 기준(수립한 완료 조건 기준 도달, 예: TC 90% 만족), 테스트 커버리지(요구사항 모두 충족), 테스트 리스크(예상 위험 제거 완료), 테스트 스케쥴(예정 일정 종료). 기본 원칙으로 요구 기능의 정상 동작·산출물 최신화(구현 관점)와 우선순위 높은 버그 100% 해결(단계 관점)이 제시됩니다. 테스트는 결함이 0이 될 때까지가 아니라(완벽 테스팅 불가 원리) 정해둔 기준 충족 시 끝낸다는 발상이 핵심 — 테스트 원리 토픽과 엮어 출제됩니다." },
  "lehman": { image: "/concept/book/lehman.png", easy: "레만의 법칙은 소프트웨어의 지속적 진화를 시스템 3유형과 8가지 원리로 설명한 변화의 법칙입니다. 시스템 유형 — S-Type(정적: 명세로 완전 정의, 변화 없음), P-Type(실용: 현실 문제 근사), E-Type(내장: 현실 세계에 박혀 계속 진화). 8원리 [계복진조 친지감피] — 계속적 변경, 복잡도 증가, 프로그램 진화, 조직적 안정화, 친근성 유지, 지속적 성장, 품질 감소, 피드백 시스템. 'E-Type만이 8원리의 대상'이라는 것과, 유지보수·리팩토링의 이론적 근거라는 점이 포인트입니다." },
  "fuzzing-test": { image: "/concept/book/fuzzing-test.png", easy: "퍼징은 제품에 랜덤·비정상 데이터를 대량 입력해 예외와 오류를 분석하고 보안 취약점을 찾아내는 테스팅 기법입니다. 절차 6단계 — 대상 분석 → 입력 값 선정 → 테스트 케이스 생성 → 실행 → 시스템 동작 모니터링 → 문제 분류·해결. 유형은 세 축으로 나뉩니다: 데이터 생성 방식(랜덤/변이/생성 기반), 데이터 투입 방식, 변조 대상. 크래시가 나면 그 지점이 곧 취약점 후보라는 것, 그리고 보안 테스트(시큐어 코딩 검증)와 짝으로 나온다는 것이 시험 포인트입니다." },
  "performance-test": { image: "/concept/book/performance-test.png", easy: "성능 테스트는 개발된 시스템이 주어진 환경에서 요구사항의 목표치를 달성하는지 확인하는 테스트입니다. 프로세스는 요구사항 정의 → 계획·설계 → 구현·수행 → 종료. 성능 지표 6종을 정확히 — 응답시간(요청 하나의 소요 시간), 시간당 처리량(Throughput), 자원사용량(Utilization), 효율성, 반환시간(Turnaround), 가용도(Availability). 유형은 테스트 방법 측면(부하·스트레스 등 7종)과 목적 측면 4종으로 표에 정리돼 있습니다. '응답시간=하나의 속도, 처리량=단위시간의 양'의 구분이 답안 기본기입니다." },
  "iso-29119": { image: "/concept/book/iso-29119.png", easy: "ISO 29119는 SW 개발 생명주기 전 과정의 테스팅 프로세스와 산출물을 다루는 소프트웨어 테스트 국제 표준 시리즈입니다. 구성 [개프도테키] — Part 1 개념과 정의, Part 2 테스트 프로세스(조직/관리/동적 3계층), Part 3 테스트 문서화, Part 4 테스트 기법, Part 5 키워드 주도 테스팅. 여기에 Part 11이 AI 시스템 테스트 지침으로 확장됐습니다. '몇 부가 무엇인가'를 두음으로 잡고, 테스트 프로세스 3계층(조직→관리→동적)이 단골입니다." },
  "iso-29119-11": { image: "/concept/book/iso-29119-11.png", easy: "ISO 29119-11은 AI 기반 시스템을 도입·테스트하는 방법의 지침을 제공하는 ISO/IEC 기술보고서 — 테스트 표준 ISO 29119의 11부입니다. AI 시스템은 정답 판정 기준(테스트 오라클)이 모호해 기존 기법이 안 통하는데, 이를 위한 블랙박스 기법으로 조합 테스트, 백투백 테스트, A/B 테스팅, 변성 테스트(Metamorphic — 입력을 변형해도 출력 관계가 유지되는지), 탐색적 테스트를 제시합니다. 'AI 테스트의 오라클 문제 → 해법이 변성·백투백'이라는 논리 흐름이 답안 뼈대입니다." },
  "mutation-test": { image: "/concept/book/mutation-test.png", easy: "뮤테이션 테스트는 소스 코드를 일정 규칙으로 변형(뮤턴트 생성)한 뒤, 원본과 다른 결과를 내는 테스트케이스를 선별하는 결함 기반 테스트입니다 — 코드가 아니라 '내 테스트의 결함 검출 실력'을 검증하는 테스트의 테스트입니다. 원본 결과 R1과 뮤턴트 결과 R2를 비교해 R1=R2면 그 케이스는 변형도 못 잡은 것(검출력 없음), R1≠R2면 뮤턴트를 잡아낸 유효한 케이스입니다. 뮤테이션 연산자는 대치·변형(치환)·삭제 세 갈래. '테스트케이스의 품질을 평가한다'는 목적이 시험 포인트입니다." },
  "test-process": { image: "/concept/book/test-process.png", easy: "테스트 프로세스는 테스트 계획부터 오류 추적·수정까지 테스트 활동을 단계화한 5단계 절차입니다(IEEE 829). [계케실결오] — 테스트 계획(범위·전략·종료기준) → 테스트 케이스 설계 → 테스트 실행 및 측정 → 결과 분석 및 보고 → 오류 추적 및 수정. '테스트도 주먹구구가 아니라 프로세스로 돈다'는 것과, 각 단계의 산출물(계획서·케이스·결과 보고서)이 짝으로 출제됩니다. ISO 29119 Part 2(테스트 프로세스)와 연결해 두세요." },
  "back-to-back": { image: "/concept/book/back-to-back.png", easy: "Back to Back 테스트는 둘 이상의 테스트 시스템(버전)에 동일한 입력을 주고 실행해 결과값을 비교하고, 불일치가 나오면 그 원인을 분석하는 기법입니다. 절차는 Test Case 작성 → 테스트 수행 → 결과값 비교 → 원인 분석. 대표 사례가 자동차 분야의 모델-코드 간 검증 — 모델 분석 → 테스트 케이스 작성 → 모델에서 수행 → 소스코드에서 수행 → 결과 비교·리포트로, 설계 모델과 생성 코드가 같은 동작을 하는지 확인합니다. '정답(오라클) 대신 서로를 비교한다'가 핵심 아이디어입니다." },
  "iso-25010": { image: "/concept/book/iso-25010.png", easy: "ISO/IEC 25010:2023은 SW 품질 특성과 평가 Metrics를 정의한 국제표준(SQuaRE 시리즈)의 2023 개정판입니다. 품질 특성 9개 — 기능 적합성, 신뢰성, 상호작용 능력(구 사용성), 성능 효율성, 유지 보수성, 유연성(구 이식성), 보안성, 호환성, 안전성(Safety, 신규). 개정 핵심 — 품질 모델 분리, 대상 변경, 주특성 추가(안전성)·변경(사용성→상호작용 능력, 이식성→유연성), 부특성 정비. '2023에 안전성이 주특성으로 승격'이 최신 출제 포인트입니다." },
  "sp-cert": { image: "/concept/book/sp-cert.png", easy: "SP 인증은 소프트웨어 기업·개발 조직의 '프로세스' 품질 역량을 심사해 등급을 부여하는 제도입니다(소프트웨어 진흥법 제21조, 인증기관 NIPA). 심사 기준 5영역 — 프로젝트 관리(계획·통제·협력업체 관리), 개발(요구사항·분석설계구현·테스트), 지원(품질보증·형상관리·측정분석), 조직관리(조직 프로세스·구성원 교육), 프로세스 개선(조직성과·문제해결·개선관리). 등급은 1~3등급으로, 2등급은 '개별 프로젝트 차원의 프로세스 수립·통제', 3등급은 '조직 차원의 프로세스 정의와 지속적 개선'이 가능한 수준입니다(인증은 2·3등급에 부여). 제품 자체를 시험하는 GS 인증과 달리 SP는 '만드는 과정의 역량'을 봅니다 — 'GS=제품, SP=프로세스' 한 줄 대비와 심사 5영역, 등급별 수준 구분이 시험 포인트입니다." },
  "gs-cert": { image: "/concept/book/gs-cert.png", easy: "GS 인증은 국산 SW 제품의 품질 향상을 위해 SW 시험 인증 센터가 국제표준 기반으로 운영하는 한국형 SW '제품' 품질 인증제도입니다. 평가 모델 [23측정 51요구 41평가] — ISO/IEC 25023(품질 측정), 25051(요구사항), 25041(평가 절차). 품질 특성은 기능적합성·성능효율성·호환성·사용성·신뢰성·보안성·유지보수성·이식성 + 일반적 요구사항. 절차는 신청·접수 → 상담 → 계약 → 품질 시험·평가 → 인증 심의 → 적합/부적합. '제품을 보는 GS vs 프로세스를 보는 SP'의 대비가 단골입니다." },
  "cmmi": { image: "/concept/book/cmmi.png", easy: "CMMI는 조직의 프로세스 개선을 통해 개발의 비용·품질·일정을 충족시키기 위해, 성숙도 레벨별 수행 활동을 제시한 모델입니다(V3.0). 구성 요소 — Category Area > Capability Area > Practices의 계층에 Domain(개발·서비스·공급자 관리 등) 개념이 더해졌습니다. 성숙도 5레벨 — 초기(1) → 관리(2) → 정의(3) → 정량적 관리(4) → 최적화(5). '조직 프로세스의 성숙도를 평가·개선한다'는 목적과 레벨별 키워드(2=프로젝트 단위 관리, 3=조직 표준, 4=정량화, 5=지속 개선)가 답안 핵심입니다." },
  "oss-governance": { image: "/concept/book/oss-governance.png", easy: "오픈소스 거버넌스는 OSS를 안전하게 사용·적용·배포하기 위한 절차와 체계를 소프트웨어 라이프 사이클 단계별로 제시한 관리 프레임워크입니다. 프레임워크 [정획적운관] 5단계 — 정책수립(전략·조직·라이선스 정책) → 획득(요구 정의·검색·검증·선정) → 적용(설계 반영·컴플라이언스 확인) → 운영 및 유지(모니터링·업데이트·취약점 대응) → 관리 및 개선(성과 측정·프로세스 개선). '라이선스 준수'와 '보안 취약점 관리'라는 두 리스크를 전 주기로 다스린다는 게 핵심이며, 오픈소스 보안위협 토픽의 해법 편입니다." },
  "sw-sizing": { image: "/concept/book/sw-sizing.png", easy: "SW 규모산정은 소프트웨어의 양적 크기와 질적 수준을 파악해 소요 공수·자원·기간을 산정하고 실행 가능한 계획을 세우는 비용 산정 과정입니다. 방법 3갈래 — 하향식(전문가 감정·델파이: 간편하나 주관적), 상향식(LOC·M/M: 세부 집계로 객관적), 수학적(FP·COCOMO·Putnam: 모델 기반). 고려사항은 프로젝트 요소·자원 요소·생산성 요소 3축. 방법별 특징·장단점·산정방식 비교표가 단골이며, '초기는 하향식, 상세화되면 수학적'이라는 적용 시점 논리까지 쓰면 완성입니다." },
  "stpa": { image: "/concept/book/stpa.png", easy: "STPA는 STAMP 이론 기반의 최신 위험분석 기법으로, 부품 고장이 아니라 시스템 요소 간 상호작용(제어)이 안전을 위협하는지 분석합니다. 절차 4단계 — 사고·위험 정의 → Control Structure(제어 구조) 도식화 → UCA(Unsafe Control Actions) 도출 → 원인 시나리오 도출. UCA 4유형이 핵심 — ① 필요한 제어가 제공되지 않음 ② 부적절한 제어 제공 ③ 너무 이르거나 늦음(타이밍) ④ 너무 짧거나 김(지속시간). 고장 없이도 사고가 나는 현대 복잡 시스템(자율주행 등)에 맞는 기법이라는 배경이 답안 차별화 포인트입니다." },
  "eta": { image: "/concept/book/eta.png", easy: "ETA는 초기 이벤트에서 출발해 후속 안전장치의 성공/실패 가지를 치며 각 시나리오의 발생 확률을 계산하는 정량적·귀납적 위험 분석기법입니다. 절차 [범위초 트결경] — 분석 대상·범위 정의 → 시스템 위험(사고) 정의 → 초기 이벤트 정의 → Event Tree 전개 → 결과 리스크 파악 → 위험 경감 대책 수립. FTA가 '결과→원인'(연역)이라면 ETA는 '원인(초기 이벤트)→결과 시나리오'(귀납)로 방향이 반대라는 것, 그리고 가지마다 확률을 곱해 시나리오 확률을 낸다는 것이 포인트입니다." },
  "hazop": { image: "/concept/book/hazop.png", easy: "HAZOP은 관련 전문가들이 모여 공정변수 × 가이드워드 조합으로 설계 의도로부터의 이탈(Deviation)을 찾고 원인·영향을 분석하는 정성적 안전성 기법입니다. 절차 — 목적·범위 설정 → 분석 팀 구성 → 예비 조사 → 토론·검토 → 결과 기록. 평가 방식이 독특합니다: 공정변수(유량·온도 같은 특정변수, 일반변수)에 가이드워드(없음 NO/NOT, 증가 MORE, 감소 LESS, 반대 REVERSE, 부가 AS WELL AS, 부분 PART OF)를 붙여 '유량이 없다면?'식으로 체계적 브레인스토밍을 합니다. '전문가 집단 + 가이드워드'가 식별 키워드입니다." },
  ...
  "ops-performance": { image: "/concept/book/ops-performance.webp", easy: "정보시스템 운영 성과관리는 전자정부법에 따라 운영 중인 시스템의 성과를 측정·평가해 정비 대상을 정하고 업무·비용 성과를 높이는 활동입니다. 추진절차 — 업무성과 계획관리 → 통폐합 대상관리 → 성과측정 대상관리 → 성과측정 및 평가 → 폐기 예외관리 → 정비계획 수립 → 이행관리. 성과측정 지표는 비용지표와 업무지표(업무성과 달성도), 정비 방식 4종 — 폐기, 통폐합, 기능고도화, 전면재개발. '측정 결과에 따라 계속/정비/폐기를 정한다'는 목적과 정비 방식 4종이 답안 핵심입니다." },
  "audit-report": { image: "/concept/book/audit-report.png", easy: "감리결과보고서는 독립된 감리법인이 제3자적 관점에서 수행한 현장감리 결과를 정리해 제출하는 최종 산출물입니다. 구성 3부 — ① 종합의견(전제조건, 총평, 감리영역별 상세점검결과 요약) ② 감리영역별 점검결과(사업관리·품질보증 / 응용시스템 / DB·시스템아키텍처·보안 등 영역별 점검항목·상세점검결과) ③ 별첨(감리 수행 전 제출한 감리계획서). 개선권고사항에는 유형(필수/협의/권고), 개선시점(장기/단기), 중요도, 발주기관 협조필요 여부를 표기합니다. 제출 프로세스는 보고서(안) 설명 → 이견접수·통보일 제시 → 이견사항 처리결과 공유 → 보고서 확정·통보 4단계. '종합의견에 무엇이 들어가나'와 개선권고 유형 3종(필수·협의·권고)이 시험 포인트입니다." },
  "common-audit-process": { image: "/concept/book/common-audit-process.png", easy: "공통감리 절차는 정보시스템 개발, EA, ISP 수립, DB 구축 등 모든 유형의 정보화 사업에 공통 적용되는 감리 절차입니다. 3단계 [예현조] — A00 예비조사(감리계획서 작성) → B00 현장감리(감리수행결과보고서) → C00 시정조치 확인(조치확인보고서). 감리법인·발주기관·피감리인(사업자) 3주체 사이의 문서 흐름(계획서 통보 → 결과보고서 제출 → 조치 → 확인)이 개념도로 출제됩니다. 단계별 산출물 짝짓기 — 예비조사=계획서, 현장감리=결과보고서, 조치확인=확인보고서 — 가 단골입니다." },
  "audit-mandatory": { image: "/concept/book/audit-mandatory.png", easy: "정보시스템 감리는 효율성 향상과 안전성 확보를 위해 제3자적 관점에서 구축 사항을 종합 점검·개선하는 활동입니다. 의무 대상(전자정부법) — 대국민 서비스 사업, 여러 기관 공동 구축, 사업비 5억 이상, 기관장이 필요하다고 인정하는 사업. 관점별 점검 기준 [성산절] — 성과(목표 달성도), 산출물(품질·완전성), 절차(프로세스 준수). 3단계 감리 수행 절차와 산출물이 짝으로 나오며, '5억'이라는 숫자와 [성산절] 두음이 답안 필수 요소입니다." },
  "ops-maintenance-audit": { image: "/concept/book/ops-maintenance-audit.png", easy: "운영/유지보수 감리는 구축 완료 후 인도된 정보시스템의 변경·개선·모니터링을 점검해, 안정적 운영과 성능 보장·효율적 개선을 확인하는 감리입니다. 점검 분야가 감리 유형별로 다른 게 포인트 — 운영 감리는 개발 소프트웨어(DS)·인프라(IF) 2개 분야, 유지보수 감리는 개발 소프트웨어(DS)·상용 소프트웨어(CS)·인프라(IF) 3개 분야를 봅니다. '구축 감리(개발 중)와 달리 운영 단계를 보는 감리'라는 위치, 그리고 운영과 유지보수의 점검 분야 차이가 시험 포인트입니다." },
  "oss-security-threat": { image: "/concept/book/oss-security-threat.png", easy: "오픈소스 SW 보안위협은 소스가 공개되어 자유롭게 사용·변경·배포되는 특성에서 생기는 위협과 그 관리방안입니다. 관리적 측면 — 사용(현황 파악 부재, 커뮤니티 코드 맹신·타이포스쿼팅)과 프로세스(점검 절차 부재). 기술적 측면 — 공격(공개 코드의 취약점 악용, Zero Day)과 조치(패치 지연). 관리방안은 사용·프로세스·개발·장비 측면으로 정리 — 오픈소스 현황 목록화(SBOM), 검증 프로세스, 취약점 스캐닝 등입니다. SBOM·오픈소스 거버넌스와 3종 세트로 묶어 외우세요." },
  "fta": { image: "/concept/book/fta.png", easy: "FTA는 최상위 사고(Top Event)에서 출발해 그 원인을 트리로 파 내려가는 연역적(Top-down) 안전성 분석 기법으로, 정성·정량 분석을 모두 지원합니다. 프로세스 6단계로 대상 정의부터 개선까지 진행하고, 표기법이 시험 단골 — 사상 기호(기본사상·중간사상·미전개사상 등)와 게이트 기호(AND: 모두 발생해야 상위 발생 / OR: 하나만 발생해도)로 트리를 그립니다. '결과에서 원인으로(연역), 게이트로 확률 계산'이 핵심이고 FMEA(귀납)와의 방향 대비가 반드시 나옵니다." },
  "iso-14764": { image: "/concept/book/iso-14764.png", easy: "ISO/IEC/IEEE 14764는 ISO/IEC 12207의 유지보수 프로세스를 6단계로 상세화한 SW 유지보수 표준 프로세스입니다. 절차 — 공정구현 → 문제 및 수정분석 → 수정 구현 → 유지보수 검토/승인 → 이전(Migration) → SW 폐기. 변경유형은 반응적(Reactive: 수정·적응)과 순향적(Proactive: 완전·예방)으로 나뉘고, 기법으로 프로그램 이해·재공학·역공학·재구조화를 씁니다. '유지보수에도 국제표준 프로세스가 있다'와 6단계 순서가 시험 포인트입니다." },
  "mccabe": { image: "/concept/book/mccabe.png", easy: "McCabe 순환 복잡도는 제어 흐름 그래프에서 독립 경로 수를 구해 SW 복잡도를 재는 지표입니다. 공식 세 가지가 모두 같은 값 — V(G) = 간선(e) − 노드(n) + 2 = 폐구간 + 1 = 의사결정 수 + 조건 수 + 1. 판정 기준 — 5 이하 단순, 5~10 안정, 20 이상 복잡(리팩토링 대상). 화이트박스 테스트에서 '기본 경로 수 = 최소 테스트케이스 수'로 직결된다는 활용처와, 간단한 그래프를 주고 복잡도를 계산시키는 문제가 단골입니다." },
  "maintenance": { image: "/concept/book/maintenance.png", easy: "유지보수는 SW 생명주기의 마지막(폐기 전) 단계로, 인도된 소프트웨어의 오류를 수정하고 요구사항을 반영하며 기능·성능을 개선하는 활동입니다. 유형 분류 3축이 핵심 — 시점 [계예응지](계획·예방·응급·지연), 대상 [데프문시](데이터·프로그램·문서·시스템), 원인 [수완예적](수정: 오류 고침 / 완전: 기능 개선 / 예방: 미래 대비 / 적응: 환경 변화 대응). 원인 축의 '수정=오류, 완전=개선, 적응=환경'구분이 단골이며, 전체 SW 비용의 60~80%가 유지보수라는 배경도 자주 나옵니다." },
  "commercial-sw-quality-test": { image: "/concept/book/commercial-sw-quality-test.webp", easy: "상용SW 품질성능 평가 시험(BMT)은 동종 경쟁 제품 간 기능·성능을 비교 평가해 요구를 만족하는 우수 제품을 가려내는 시험입니다(소프트웨어 진흥법 제55조). 평가 대상은 경쟁입찰을 통한 직접구매 대상 상용SW 중 구매금액 1억 원 이상(VAT 포함)으로 대상품목(34종)에 해당하는 제품이고, 시험비용 대비 효과가 낮거나 기존 제품 증설이면 제외될 수 있으며, 정보보호제품 성능평가 결과로 대체도 가능합니다. 절차는 평가시험 대상 검토 → 사전 협의 → 평가시험 설계(요구사항 분석·평가항목 개발) → 조달 발주(제안요청서에 평가항목·배점 반영) → 시험 의뢰 → 실시 → 결과 반영(기술성평가 반영·우선협상대상자 선정) 7단계. '스펙 문서가 아니라 실측으로 비교한다'는 취지와 상용SW 직접구매 제도와의 연계가 포인트입니다." },
  "function-point": { image: "/concept/book/function-point.png", easy: "Function Point는 정보처리 규모와 기능 복잡도에 의거해 SW 규모를 재는 정량적 산정 방식입니다 — 코드 줄 수가 아니라 '사용자에게 보이는 기능 수'로 잽니다. 산정 절차: 측정 유형 결정 → 범위·경계 식별 → 데이터 기능(ILF·EIF) 측정 → 트랜잭션 기능(EI·EO·EQ) 측정 → 미조정 FP 결정 → 조정인자(14개, 0~5점) → 조정 FP = 미조정 × 조정인자. 데이터 기능(내부논리파일·외부연계파일)과 트랜잭션 기능(입력·출력·조회)의 5요소 분류, 그리고 SW 사업대가 산정의 기준이라는 활용처가 시험 포인트입니다." },
  "sw-cost": { image: "/concept/book/sw-cost.png", easy: "SW 사업대가는 공공 SW사업의 예산 수립·발주·계약 때 적정 대가를 산정하는 기준 가이드입니다('25년 개정판, 1FP=605,784원). 사업 생애를 기획 → 구현 → 운영 단계로 나눠 각각 다른 산정 방식을 씁니다. 기획 단계(ISP·ISP/BPR·EA/ITA·ISMP·정보보안컨설팅)는 컨설팅 업무량 방식(업무 가중치×난이도)이나 투입공수 방식(직접 인건비 + 제경비 144~154% + 기술료 20~40% + 직접경비)으로, 구현 단계(SW 개발)는 기능점수 방식으로 계산합니다. 기능점수 절차는 [사기전후직소] — 사전준비(정통법/간이법 결정) → 기능점수 산정 → 보정 전 개발원가(FP×단가) → 보정 후 개발원가(규모·연계·성능·호환성·보안 보정계수) → 직접경비·이윤(개발원가의 25% 이내) → 개발비 확정. '기획은 공수, 구현은 기능점수' 구분과 이윤 25% 숫자가 시험 포인트입니다." },
  "obfuscation": { image: "/concept/book/obfuscation.png", easy: "난독화는 코드의 가독성을 의도적으로 낮춰 역공학을 어렵게 만드는 보호 기법입니다. 기술 분류 [구데집제예] — 구획(layout: 주석 제거·식별자 훼손, 성능 저하 없음), 데이터(data: 변수·자료구조 변환), 집합(aggregation: 메소드·클래스 병합/분할), 제어(control: 제어 흐름 왜곡, 성능 저하 있음), 예방(preventive: 역난독화 도구 무력화). 기법별 특징·장단점 비교표가 출제되며, '구획은 성능 영향 없음, 제어는 성능 저하 감수'의 트레이드오프와 모바일 앱·라이선스 보호라는 활용처가 포인트입니다." },
  "usability-eval": { image: "/concept/book/usability-eval.png", easy: "사용성 평가는 사용자가 실제 제품을 쓰는 모습을 관찰·분석해 효율성, 학습 용이성, 문제점과 개선 요구를 찾아내는 공학적 테스트입니다. 절차 — 계획 수립 → 평가 설계 → 평가 실행 → 분석/보고. 유형 4가지 — 탐색적(초기 설계안 탐색), 평가(프로토타입 사용성 측정), 검증(출시 전 기준 충족 확인), 비교(대안 간 비교). 평가 항목·지표는 작업시간, 사용패턴, 정확성, 완성도, 학습 용이성, 일관성 등 정량 지표로 잽니다. 'ISO 9241의 효과성·효율성·만족도'와 연결하면 답안이 풍성해집니다." },
  "sbom": { image: "/concept/book/sbom.png", easy: "SBOM은 소프트웨어를 구성하는 컴포넌트·구성 요소를 식별 메타데이터, 저작권·라이선스 정보와 함께 목록화한 공식 SW 자재 명세서입니다 — 식품의 성분표에 해당합니다. 기술 요소 두 축: Baseline Attributes(작성자, 타임스탬프, 공급자명, 컴포넌트명, 버전, 해시, 고유 식별자, 의존 관계 등 최소 필수 항목)와 표준 포맷 3종(SPDX, CycloneDX, SWID). Log4j 사태처럼 취약점이 터졌을 때 '우리 제품에 그 컴포넌트가 있는가'를 즉시 추적하는 공급망 보안의 핵심 수단이라는 맥락이 답안 포인트입니다." },
  "direct-purchase": { image: "/concept/book/direct-purchase.png", easy: "상용SW 직접구매(분리발주)는 공공 정보화사업에서 HW·SW·시스템통합 일괄 발주에 묻히지 않게, 발주기관이 상용SW만 별도로 발주·평가·선정·계약하는 제도입니다(SW진흥법 54조). 직접구매 대상과 제외 기준(사업 특성상 분리가 곤란한 경우 등)이 표로 정리되어 있고, 법적 근거는 SW진흥법과 SW사업 계약·관리감독에 관한 지침입니다. 취지 — SI 대기업 하도급 구조에서 상용SW 기업을 보호하고 제값을 받게 한다 — 와 '분리발주'라는 별칭이 시험 포인트입니다." },
  "sw-impact-assessment": { image: "/concept/book/sw-impact-assessment.webp", easy: "SW사업 영향평가는 국가기관 등이 SW 사업의 예산편성·발주나 SW 배포·서비스 제공을 추진할 때, 민간 SW 시장 침해 여부를 사전 검토·조정하는 제도입니다(SW진흥법 43조). 절차는 예산편성 단계의 자체 평가부터 발주 전 재평가, 이의신청·조정까지 5단계 흐름. 대상은 국가기관 등의 SW 사업이고, 제외 사업도 규정돼 있습니다. '공공이 민간 시장을 침범하지 않게 하는 장치'라는 취지와, 예산 단계와 발주 단계에서 두 번 본다는 구조가 답안 포인트입니다." },
  "feasibility-study": { image: "/concept/book/feasibility-study.png", easy: "정보화사업 예비타당성 조사는 국가재정법 제38조에 따라 대규모 신규 사업의 예산 편성·기금 운용계획 수립 전에 기획재정부장관 주관으로 실시하는 사전 타당성 검증·평가 제도입니다. 대상 기준 [사5지 3신] — 총사업비 500억 원 이상이면서 국가 재정지원 300억 원 이상인 신규사업(타당성조사비·설계비 등 국고지원이 없었던 사업). 필요성은 세 측면 — 경제적(수요·경제성 없는 사업의 추진 가능성 제거로 예산 낭비 최소화), 기술적(사업비 증액·계획 변경 리스크 완화, 중도 취소 방지), 정책적(후보사업군 비교·검토로 사업 우선순위 결정). '예산이 편성되기 전 단계의 검증'이라는 위치(ISP·ISMP보다 앞)와 '사5지3신' 숫자가 시험 포인트입니다." },
  "sw-safety-guideline": { image: "/concept/book/sw-safety-guideline.png", easy: "SW 안전 확보 지침은 SW 오작동으로 인한 생명·신체·재산 피해를 막기 위해, 안전 책임자와 안전관리 대상 SW의 개발·운영 단계별 관리기준을 정한 고시입니다(SW진흥법 30조 2항). 주요 용어 — 위험원(피해를 유발할 수 있는 잠재 요인), 장애, 소프트웨어 안전. 지침 구성 — 1장 총칙, 2장 SW 개발단계 안전확보(위험원 분석·안전 요구사항), 3장 SW 운영단계 안전확보(모니터링·변경 관리), 4장 그 외 사항. '개발단계와 운영단계로 나눠 관리기준을 둔다'는 구조가 시험 포인트입니다." },
...
"saga-pattern": { image: "/concept/book/saga-pattern.png", easy: "SAGA는 MSA에서 서비스별 로컬 트랜잭션을 순차 실행하다가 중간에 실패하면, 앞서 성공한 서비스들에 보상 이벤트를 보내 되돌림으로써 전체 일관성을 지키는 분산 트랜잭션 패턴입니다. 서비스마다 DB가 따로라 '주문-결제-재고'를 한 트랜잭션으로 묶을 수 없기 때문입니다. 방식이 둘 — Choreography(지휘자 없음): 각 서비스가 이벤트를 Kafka 같은 메시지 큐로 전파하며 릴레이. 단순하지만 흐름 추적이 어려움. Orchestration(지휘자 있음): SAGA Manager 인스턴스가 중앙에서 트랜잭션을 요청·완료 수신. 흐름이 명확하지만 매니저가 단일 장애점이 될 수 있음." },
...
"sw-arch-process": { image: "/concept/book/sw-arch-process.webp", easy: "요구사항에서 출발해 아키텍처를 설계하고 검증·승인하는 표준 절차 4단계입니다. ① 요구사항 분석 — 기능/비기능 요구를 식별·명세·검증. ② 아키텍처 분석 — 품질속성을 식별하고 우선순위를 매김(여기가 아키텍처의 진짜 출발점). ③ 아키텍처 설계 — 이해관계자별 관점(view) 정의 → 아키텍처 스타일 선택(pipe-filter, mvc, layer 등 혼용) → 후보 아키텍처 도출(배경도·관점별 다이어그램·SAD 작성). ④ 검증 및 승인 — 아키텍처 평가(요구 만족도·품질속성 간 관계) → 상세화 반복 → 최종 승인. 설계와 평가를 반복하는 게 포인트입니다." },
...
"quality-attribute-scenario": { image: "/concept/book/quality-attribute-scenario.webp", easy: "품질 요구를 검증 가능하도록 6개 항목으로 구조화해 적는 명세 방법입니다. '가용성이 높아야 한다' 같은 말은 검증할 수 없으니 이렇게 쪼갭니다 — 자극 유발원(누가), 자극(무엇이 일어나서), 환경(어떤 상황에서), 대상(무엇에게), 응답(시스템이 어떻게 반응하고), 응답 측정(그걸 어떻게 숫자로 확인하나). 예를 들어 가용성이라면 '외부에서(유발원) 예기치 못한 메시지가 와서(자극) 정상 운영 중(환경) 프로세스에(대상) 운영자에게 통지 후 계속 수행하며(응답) 정지 시간 0(응답 측정)'. 마지막 응답 측정이 있어야 나중에 검증이 됩니다." },
...
"norm-reg-std": { image: "/concept/book/norm-reg-std.png", easy: "셋 다 머신러닝 학습을 안정시키는 손질 기법인데, 손대는 대상이 다릅니다. 정규화(Normalization)는 입력 데이터를 특정 구간(주로 0~1)으로 압축합니다. Min-Max 스케일링 (x−min)/(max−min) — 거리 기반 알고리즘 성능 향상, 단 이상치에 취약. 표준화(Standardization)는 입력 데이터를 평균 0·표준편차 1의 표준정규분포로 변환합니다. Z-score (x−μ)/σ — 경사하강법 속도 향상, 이상치 영향 축소. 규제화(Regularization)는 데이터가 아니라 모델 가중치에 패널티를 더해(J(w)=MSE+λ‖w‖²) 과적합을 방지합니다 — L1(Lasso)·L2(Ridge)가 대표. 한 줄 정리: 정규화·표준화=입력 데이터 손질, 규제화=모델 복잡도 단속." },
...
"nw-cdn": { image: "/concept/book/nw-cdn.png", easy: "CDN은 콘텐츠를 미리 여러 지역의 캐시 서버(PoP)에 복제해 두고 사용자와 가장 가까운 곳에서 꺼내주는 콘텐츠 전송망입니다. 원 서버 한 곳에서만 내려주면 멀리 있는 사용자는 느리기 때문입니다. 동작 흐름 4단계가 시험 포인트 — ① 사용자가 CP 웹서버 접속 → ② CDN 서버 주소가 박힌 Embedded URL이 든 HTML을 받음 → ③ 사용자가 그 CDN 주소로 오브젝트 요청 → ④ CDN이 복제본을 전달. 구간 이름도 함께 외우세요: 사용자쪽부터 Last-mile / Middle-mile / First-mile입니다. 기술 요소 중 헷갈리는 쌍이 있습니다 — Global Server Load Balancing은 '전 세계 캐시 서버 중 어느 서버를 붙일까'를 고르는 것이고, Load Balancing은 고른 서버군 안에서 트래픽을 나누는 것, Request Routing은 부하를 보고 가장 인접한 캐시를 선택하는 방법입니다. Grid Delivery는 트래픽이 넘칠 때 사용자 PC를 작은 서버로 쓰는 P2P 방식입니다." },
