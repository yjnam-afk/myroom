...
export const EXTRAS: Record<string, SubnoteExtra> = {
  ...
  "refactoring": { image: "/concept/book/refactoring.png", easy: "리팩토링은 모듈의 외부 기능은 그대로 두고 내부 구조·관계만 단순화해 유지보수성을 높이는 기법입니다. 대상(코드 악취) [중긴큰긴산] — 중복된 코드, 긴 메소드, 큰 클래스, 긴 파라미터 리스트, 산탄총 수술(한 변경이 여러 클래스 수정 유발). 기법은 결합도 측면(클래스 이동 등), 응집도 측면(메소드 추출 등), 단순화, 은닉으로 분류됩니다. '기능 변경 없음'이 정의의 생명이고, 테스트(회귀)와 세트로 진행해야 안전하다는 것까지 쓰면 완성입니다." },
