...
"sw-arch-process": {
    guide: {
      ...
      scene: "건축가는 건축주의 핵심 요구(채광·내진)를 먼저 추리고, 그에 맞는 공법을 골라 설계하고, 도면으로 남기고, 구조 검토를 받습니다. 아키텍처 구축 절차가 딱 이 순서로, 품질 요구(드라이버)를 뽑고 스타일·패턴을 골라 설계(ADD)하고 관점별 도면(4+1 뷰)으로 남긴 뒤 평가(ATAM·CBAM)로 검증하며, 이를 반복합니다.",
...
"se-67": {
    guide: {
      ...
      scene: "궁궐은 한가운데에 임금이 있고 바깥 행랑채는 자주 바뀌어도 안채는 그대로이며, 출입과 보고는 언제나 바깥에서 안쪽으로만 향합니다. 클린 아키텍처가 딱 이 구조로, 핵심 업무 규칙(엔티티·유스케이스)을 중심에 두고 자주 바뀌는 것들(DB·웹 틀·UI)을 바깥에 둬 의존이 항상 안쪽만 향하게 하니, 바깥을 통째로 갈아도 중심은 안전합니다.",
...
"sw-arch-driver": {
    guide: {
      ...
      scene: "집을 지을 때 벽지 색은 나중에 바꿔도 되지만 '3층으로, 지진에 견디게, 예산은 얼마까지'는 뼈대를 결정합니다. 아키텍처 드라이버가 딱 이런 요구로, 핵심 기능과 품질 속성(성능·가용성 등 — 가장 중요), 협상 불가한 제약을 먼저 추려 설계 방향(스타일·패턴 선택)을 이끌게 합니다.",
...
"utility-tree": {
    guide: {
      ...
      scene: "'좋은 집'이라는 막연한 바람은 따뜻함→단열→'한겨울에도 실내 20도'처럼 가지를 쳐 내려가야 손에 잡힙니다. 유틸리티 트리가 딱 이 가지치기로, 품질(뿌리)에서 속성(성능·보안)을 거쳐 구체 시나리오(잎)까지 내려간 뒤 잎마다 중요도·난이도(H/M/L)를 매겨, 아키텍처 평가(ATAM)에서 어디부터 볼지 정합니다.",
