...
"db-query-offloading": { image: "/concept/book/db-query-offloading.png", easy: "DB 부하의 정체는 대부분 읽기입니다 — Read가 70~90%, Update는 10~30%뿐이거든요. 그래서 쓰기는 Master DB 한 곳에서만 받고, 읽기는 여러 대의 Slave DB로 흘려보내(오프로딩) 처리량을 늘리는 기법입니다. 구성요소 [마스슬C로] — Master DB(Update만), Staging DB(중간 경유지 — Master가 다수 Slave로 직접 복제할 때의 성능저하 방지), Slave DB(Read만, N개 구성 + 장애 시 다른 인스턴스로 접근하는 HA), CDC(Source DB의 Back Log를 읽어 Target에 replay — Golden Gate, Share Flex, Galera), load balancing(Slave 조회 부하 분산). 샤딩과 헷갈리기 쉬운데, 쿼리 오프로딩은 '트랜잭션을 유형별로 분리'(성능 향상 목적)이고 샤딩은 '데이터를 여러 인스턴스로 분할'(용량한계 극복 목적)입니다." },
"db-partitioning": { image: "/concept/book/db-partitioning.png", easy: "2025.01 ITPE FR과 119회 컴시응에 나온 기출입니다. 대규모 테이블을 파티션 키 기준으로 물리적 세그먼트 여러 개에 나눠 담는 설계 기법 — 샤딩과 달리 한 서버(한 DB 인스턴스) 안에서의 분할입니다. 테이블 파티셔닝 유형 6가지: 레인지(키 값 범위로 — 가장 일반적), 리스트(불연속 값 목록을 지정 — 서울/경기 같은 그룹핑), 해시(해시 함수로 고르게 분산 — 병렬 처리 유리), 결합(레인지 후 해시/리스트로 서브 파티션), Reference(부모 테이블 키로 자식 테이블 파티셔닝), Interval(정해진 간격으로 자동 확장). 인덱스도 파티셔닝 — 글로벌(여러 파티션이 공유)과 로컬(파티션마다 개별). 설계 절차는 액세스 패턴 분석 → 데이터 분포 분석 → 인덱스 설계 → 테이블 파티셔닝 설계 → 인덱스 파티셔닝 설계 순입니다." },
"db-sharding": { image: "/concept/book/db-sharding.png", easy: "127회 정보관리 4교시 기출입니다. 데이터를 샤드(Shard)라는 개별 파티션으로 수평 분할해 물리적으로 다른 DB 서버들에 나눠 담는 기법 — 파티셔닝이 '한 인스턴스 안 분할'이라면 샤딩은 '여러 인스턴스로 분할'입니다. MongoDB 그림처럼 Application → MongoS(라우터) → Shard1·2·3 구조로, shard key(어느 샤드로 갈지 정하는 칼럼)와 proxy(힌트+메타데이터로 요청을 해당 샤드에 전달)가 핵심 구성요소입니다. 분할방법 [해레디] — Hash(해시로 균일 분산, 샤드 추가 시 재정렬 필요), Range(범위 기준, 일부 샤드에 데이터 집중 가능), Directory(별도 조회 테이블로 라우팅, 그 테이블이 단일 장애 포인트). 비교표의 '샤딩=수평 분할·별도 서버·Master Node 관리 / 파티셔닝=수평·수직·동일 서버·Master 없음'이 답안 단골입니다." },
"db-data-standard": { image: "/concept/book/db-data-standard.png", easy: "시스템마다 같은 데이터를 '고객명', 'CUST_NM', 'customer'로 제각각 부르면 불일치가 생기고, 전사 DW를 만들 때 의미 파악·중복 파악이 안 되고, 유지보수 때마다 고생합니다 — 그래서 명칭·정의·형식·규칙 4가지 원칙을 세워 전사적으로 적용하는 활동이 데이터 표준화입니다. 표준화 대상 4가지: 명칭(유일하게 구별해주는 이름), 정의(의미하는 범위·자격요건), 형식(표현 형태 정의로 입력 오류 최소화), 규칙(가능한 값을 사전 정의). 이를 떠받치는 체계 3가지: 데이터 표준(표준용어·표준단어·표준 도메인·표준코드), 데이터 표준 관리조직(데이터 관리자가 정의·체계화·감독·보안 담당), 데이터 표준화 프로세스(요구사항 수집 → 표준 정의 → 표준 확정 → 표준 관리 순)입니다." },
"db-governance": { image: "/concept/book/db-governance.png", easy: "데이터를 '잘 쓰는' 것보다 먼저 '잘 다스리는' 체계입니다. 전사 차원에서 정책·지침·표준화·전략을 세우고, 관리 조직과 프로세스를 구축해 고품질 데이터로 가치 창출을 지원합니다. 프레임워크는 신전 그림으로 기억하세요 — 지붕(데이터 관리 원칙), 그 아래 데이터 관리 조직, 여섯 기둥(데이터 표준·구조·흐름·품질·베이스·보안), 바닥(데이터 관리 프로세스와 인프라). 주요기능 [품메주보] — 품질 관리(DQM: 프로파일링·정제), 메타 데이터 관리, 데이터 주기 관리(생성부터 폐기까지), 보안 및 프라이버시(역할 기반 보호 수준). 성숙도는 도입 → 프로세스화 → 통합경영 → 위험대응 → 가치창출 5단계로, 단계가 오를수록 품질·통제·조직 부문이 전사 전략과 하나로 묶입니다." },
...
"db-eda-cda": { image: "/concept/book/db-eda-cda.png", easy: "탐정과 판사의 비유가 핵심입니다. EDA(탐색적)는 Statistics as Detective — 데이터를 먼저 수집해 시각화로 탐색하면서 패턴을 찾고 '가능성이 큰 가설'을 도출합니다(데이터 → 모형, Exploratory). CDA(확증적)는 Statistics as Judge — 가설을 먼저 세우고 데이터를 모아 통계 분석으로 P-value 기준 수용/기각을 판정합니다(모형 → 데이터, Confirmatory). 기법도 대비되게: EDA는 히스토그램·줄기잎그림·상자수염그림·산점도 같은 시각화 도구, CDA는 t-test·F-test·ANOVA·상관/회귀분석·카이제곱검정 같은 검정 도구. 핵심요소는 EDA가 저항성·잔차 해석·재표현·현시성, CDA가 중심극한의 정리·P-value입니다. 데이터 분석 거버넌스의 프로세스 항목(EDA/CDA)과 연결해 두면 좋습니다." },
"db-visualization": { image: "/concept/book/db-visualization.png", easy: "숫자 뭉치를 '한눈에 보이는 이야기'로 바꾸는 프로세스입니다. 시각화 프로세스 [구시표] 3단계 — ① 정보 구조화(탐색·분류·배열로 그룹핑: 데이터 수집·분류(JSON/XML/CSV)·배열(래치 방법: 위치·알파벳·시간·카테고리·위계)·관계 탐색), ② 정보 시각화(막대·파이차트·스캐터·버블·히스토그램으로 시간·분포·관계를 직관적으로 표현), ③ 정보 시각 표현(타이포그래피·색상·그리드 같은 디자인 원리와 인터렉션 디자인으로 완성). 시각화로 인사이트를 얻는 절차는 [탐분활] — 탐색(데이터 확인·연결포인트·관계 탐색) → 분석(분석대상 정의·시각화 도구·지표설정) → 활용(내부적용·외부적용·인사이트 발전과 확장)입니다. '구조화 없이 그래프부터 그리면 안 된다'는 흐름이 출제 포인트입니다." },
...
"db-daap": { image: "/concept/book/db-daap.png", easy: "2025.11 모의고사 기출 최신 토픽. 데이터를 창고에 쌓인 '자산'이 아니라 '사용자를 가진 제품'으로 관리하자는 패러다임입니다. 계층 구조: 원천 데이터(Source: 트랜잭션DB·Log·IoT센서) → 데이터플랫폼&거버넌스(표준화 저장소·CI/CD·접근제어) → 데이터제품(데이터셋+메타데이터+품질지표, API 제공, SLA 보장, Owner 지정!) → 데이터소비자(BI·AI모델·앱). 핵심원칙 4가지 — 도메인 소유권(데이터를 가장 잘 아는 팀이 직접 관리, 데이터 메시의 원칙), 데이터 제품('데이터셋'이 아니라 '사용자를 가진 제품'), 표준화&상호운용성(도메인 간 호환 공통 규칙), 관측 가능성(품질·지연·스키마 변동 모니터링). 데이터 메시·패브릭·리니지·카탈로그 같은 키워드와 세트로 출제됩니다." },
"mg-tech-debt": { image: "/concept/book/mg-tech-debt.png", easy: "빚을 지면 이자가 붙듯, '일단 돌아가게만' 만든 임시방편 코드는 나중에 더 큰 위험 비용으로 돌아옵니다. 원인 4가지 — Business Pressure(무리한 일정·적은 예산), Low Technology Maturity(잘못된 아키텍처·경험 부족), Frequent Requirement Changes(코드 복잡성 상승·낮은 테스트 커버리지), Lack of Experts(부족한 테스팅·스파게티 코드). 유형은 설계(모듈성 부족)·코드(정적 분석/컨벤션 위반)·테스트(방법론 부재)·문서(산출물 누락) 부채 4가지. 관리방안은 부채 추정(SonarQube·Coverity 정적 분석) → 우선순위화 → 템플릿 통합(Prettier·Lint) → 기술 업데이트(도커·k8s) → 리팩토링과 테스팅(Peer Review·Inspection). 사분면 그림 — 보이는 긍정=Feature, 안 보이는 긍정=Architecture, 보이는 부정=Bug, 안 보이는 부정=Technical Debt — 가 시험 단골입니다." },
...
"mg-slm": { image: "/concept/book/mg-slm.png", easy: "SLA가 '계약서'라면 SLM은 그 계약이 지켜지는지 사용자 관점에서 재고 개선하는 '관리 체계'입니다. 개념도 구조가 핵심 — 사용자·고객은 관리조직과 SLA로 소통(SLA 협상·수준 예측·보고)하고, 관리조직은 운영조직과 OLA로 소통(서비스 운영·수준 관리·측정)합니다. 구성요소 [카스(에)오 플로(리)엔] 6가지 — Service Catalog(제공 서비스 전체 목록), SLA(외부 업체 간 계약서), OLA(내부 부서 간 협약서 — SLA와의 내/외부 구분이 단골 문제!), Service Quality Plan(수준 보장 내부 계획), Service Report(주기적 위반 검토), SLM 엔진(지표별 측정치 산출·보고 자동화·실시간 모니터링). 측정지표는 HW(가동률·동일장애발생률), SW(오류건수·SR 적기 처리율), NW(가동률·장애건수)로 ITSM 지표와 세트로 외우면 됩니다." },
...
"mg-env-analysis": { image: "/concept/book/mg-env-analysis.png", easy: "경영전략 수립의 출발점 — 바깥과 안을 나눠서 봅니다. 외부환경분석은 [기회/위협]을 찾는 것: 절차는 거시 환경분석→산업 구조분석→경쟁 환경분석→산업 진화분석→시사점도출이고, 기법은 PEST(정치·경제·사회문화·기술의 거시 환경), 5 Force(산업 내 경쟁자 상호작용으로 기회·위협요인 도출), 3C(고객·경쟁사·자사 비교로 차별화 포인트 분석). 내부환경분석은 [강점/약점]을 찾는 것: 사업성과·재무성과 분석→경영 현황 진단→내부 역량 분석→시사점 도출 순이며, 기법은 Value Chain(가치 창출 활동의 강약점·비용 우위), BSC(재무+비재무 관점을 측정 가능한 지표로), 7S(Hard/Soft Element로 조직 총체 역량 진단), 가능역량 자원분석(자원·핵심역량·경쟁우위). 외부 기회/위협 + 내부 강점/약점 = SWOT으로 이어집니다." },
"mg-ansoff": { image: "/concept/book/mg-ansoff.png", easy: "2025.10 모의고사 기출. 성장 방향을 '제품(기존/신규) × 시장(기존/신규)' 두 축으로 정하는 의사결정 기법입니다 [시제 침개개다]. 기존 제품을 기존 시장에 더 팔면 시장 침투(위험 최소), 기존 제품을 새 시장으로 가져가면 시장 개발, 기존 시장에 새 제품을 내면 제품 개발, 새 제품으로 새 시장에 진입하면 다각화(위험 최대) — 원점에서 멀어질수록 위험도가 커진다는 축의 의미까지 쓰면 완성입니다. 같은 슬라이드의 BCG 매트릭스(사업 포트폴리오 관리)·GE 매트릭스(다차원 포트폴리오 전략)와 묶어 '성장 방향은 Ansoff, 사업 선택과 집중은 BCG/GE'로 역할을 구분해 기억하세요." },
...
"mg-protocol-economy": { image: "/concept/book/mg-protocol-economy.png", easy: "배달앱 같은 플랫폼 경제의 문제 — 플랫폼 사업자가 중간에서 수수료를 독식하고 노동자·소상공인은 막대한 노동을 제공하는 구조 — 를 블록체인으로 풀자는 대안입니다. 개인 간 프로토콜(약속)을 정해 중개자 없이 직접 거래하고, 생산자와 소비자가 '공정한 대가'를 주고받는 상생 생태계를 만듭니다. 핵심요소는 탈중앙화·탈독점화·공정한 분배(분권화). 주요특징 3가지 — 개별성(중개자 없이 판매·대여·공유), 투명성(정보 공개로 신뢰 형성), 공정성(공정한 기회와 인센티브). 적용사례로 디지털 자산거래, 마이데이터 사업, BaaS(서비스형 블록체인), De-Fi 사업이 나오며, 플랫폼 노동자·전통 산업과의 상생모델, 공유경제 활성화 모델 등이 주요 서비스 분류입니다." },
"mg-intention-economy": { image: "/concept/book/mg-intention-economy.png", easy: "광고로 시선을 붙잡는 주목 경제(Attention Economy)의 반대편 — 소비자가 '러닝이 편안하려면?'이라고 의도를 직접 말하면 기업이 'A 운동화 어떠세요?'로 맞춰 반응하는 경제입니다. 비교표가 핵심: 주목 경제는 제품 노출로 시선 끌기·수동적 소비자·자극적 콘텐츠와 Funnel 분석, 의도 경제는 의도와 목적을 먼저 파악·능동적 소비자·개인 맞춤형 추천과 의도 기반 마케팅. 구성요소는 3그룹 — 대화(프롬프트 + LLM), 의도 분석(Intention Detection Model + 인텐토노미: 2021년 메타가 발표한 인간 의도 이해용 데이터 세트), 추천 시스템(컨텐츠 기반 추천 + 협업 필터링). LLM 시대에 다시 뜨는 개념이라 최신 트렌드 문제로 나오기 좋습니다." },
