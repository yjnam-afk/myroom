...
export const EXTRAS: Record<string, SubnoteExtra> = {
  ...
  "usability-eval": {
    guide: {
      hook: "사용자가 SW를 '얼마나 쉽고 만족스럽게' 쓰는지 평가하는 활동입니다.",
      scene: "기능이 많아도 쓰기 어려우면 실패입니다. 사용성 평가는 실제 사용자가 과업을 수행하게 하거나 전문가가 검토해, 배우기 쉬운지·효율적인지·만족스러운지를 측정하고 개선점을 찾습니다.",
      ...
      map: [
        { as: "과업 완수", real: "효과성", note: "" },
        { as: "적은 노력·시간", real: "효율성", note: "" },
        { as: "실제 사용자 관찰", real: "사용성 테스트", note: "" },
        { as: "전문가 10원칙 검토", real: "휴리스틱 평가", note: "닐슨" },
      ],
...
"transfer-learning": {
    ...
    }, image: "/concept/book/transfer-learning.png", easy: "이미 잘 배운 모델의 지식을 가져와 내 문제에 맞게 조금만 더 학습시키는 기법입니다. 수백만 장으로 학습된 모델의 아래 계층은 가중치를 고정(Freeze)해 그대로 재사용하고, 위 계층만 내 데이터로 다시 학습(Fine Tuning)합니다 — 데이터가 부족하거나 훈련 시간을 줄여야 할 때 실무에서 가장 먼저 꺼내는 카드입니다. 주요 기법 [파프도레] — 파인튜닝, 프리트레인드 모델, 도메인 적응, 레이어 재사용. 유형 [적태도 레귀변자] — 적용 범위로는 과업(Task) 전이(영상인식→음성인식처럼 응용분야가 바뀜)와 도메인 전이(영불번역기→영한번역기처럼 데이터 분포가 다름), 데이터셋 레이블 여부로는 귀납(Inductive)·변형(Transductive)·자율(Unsupervised)입니다." },
"self-supervised": {
    guide: {
