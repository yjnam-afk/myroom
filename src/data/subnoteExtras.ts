...
export const EXTRAS: Record<string, SubnoteExtra> = {
  ...
  "lehman": { image: "/concept/book/lehman.png", easy: "사용되는 소프트웨어는 필연적으로 계속 진화한다는 사실을 정리한 변화의 법칙 묶음입니다 — 계속 사용되는 SW는 계속 변경되고(지속 변경), 변경할수록 구조가 복잡해지며(엔트로피 증가), 그대로 두면 품질이 하락합니다. 유지보수·리팩토링이 필연이라는 결론의 이론적 뿌리입니다." },
