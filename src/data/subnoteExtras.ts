...
"al-bubble-sort": { image: "/concept/book/al-bubble-sort.png", easy: "옆에 있는 두 값을 비교해 순서가 틀렸으면 교환하는 일을 끝까지 반복하는, 가장 단순한 정렬입니다. 한 바퀴(1회전) 돌면 가장 큰 값이 거품처럼 맨 뒤로 확정되고, 정렬되지 않은 구간을 하나씩 줄여 n−1회전 반복합니다. 구현이 가장 쉬운 대신 비교와 교환이 매번 일어나 최선·평균·최악 모두 O(n²)입니다 — 교재 빅오 슬라이드에서 O(N²) 제곱형의 대표 사례로 등장합니다. 시험에서는 '2중 loop = O(N²) = 버블 정렬'로 연결해 두면 됩니다." },
