...
export const SUBNOTES: TextbookSubnote[] = [
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["메모리 관리", "가상 메모리\n페이지 교체", "페이징 주소 변환\n단편화 관리 회수"],
          ["입출력(I/O) 관리", "드라이버 DMA\nVFS 파일", "장치 데이터 교환\n파일 접근 제공"],
          ["시스템 콜(System Call)", "Trap 전환\n커널 API 제공", "유저→커널 전환\nHW 자원 보호"],
        ],
      },
      {
        caption: "커널 구조의 유형 — 단일·계층·마이크로",
        headers: ["구조 유형", "키워드", "설명"],
        rows: [
          ["단일(Monolithic) 구조", "모든 기능 통합\n커널 비대화", "단일 커널 구성\n성능 좋으나 버그"],
          ["계층(Layered) 구조", "계층 분리 구성\n하위 서비스 이용", "콜→관리→HW\n상위 층이 호출"],
          ["마이크로(Micro) 구조", "메모리 관리 등\n사용자 영역 서버", "IPC 등만 커널\n커널 작아 안정"],
        ],
      },
      {
        caption: "커널의 종류",
        headers: ["종류", "키워드", "설명"],
        rows: [
          ["모놀리틱(Monolithic) 커널", "모든 기능 내장\n강력한 추상화", "SW 직접 제어\nLinux 적용"],
          ["마이크로(Micro) 커널", "기본 기능만\n서버 경유", "커널 경량화\nApp 제어"],
          ["하이브리드(Hybrid) 커널", "단일+마이크로\nWindows", "합성 성능 개선\nmacOS 사례"],
          ["엑소(Exo) 커널", "약한 추상화\n경량 커널", "라이브러리 제어\n임베디드 OS"],
          ["유니(Uni) 커널", "단일 주소 공간\n커널별 기능", "App 파일 장치\n경량화 구성"],
        ],
      },
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["구분", "키워드", "설명"],
        rows: [
          ["초기화", "설정값 지정", "타이머 값 초기화"],
          ["시작", "타이머 시작", "0까지 카운트"],
          ["Kick", "워치독 Kick", "태스크 주기 동작"],
          ["중단", "태스크 폭주", "리프레시 중단"],
          ["만료", "타이머 만료", "미갱신으로 만료"],
          ["리셋", "Reset 신호", "MCU 리셋 수행"],
        ],
      },
      {
        caption: "구현 방법",
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["③ PCB1 저장", "OS 문맥 저장", "P1 상태 기록"],
          ["④ PCB2 적재", "OS P2 적재", "CPU 할당 실행"],
          ["⑤ 실행", "P2 CPU 할당", "P2 실행 상태"],
          ["⑥ Interrupt/System call", "인터럽트\n시스템 호출", "운영체제 개입\nP2 대기 전환"],
          ["⑦ PCB2 저장", "OS 상태 저장", "P2 상태 기록"],
          ["⑧ PCB1 적재", "OS P1 복구", "CPU 할당 실행"],
          ["⑨ 실행", "P1 재실행\n시스템콜 I/O", "CPU 재할당\n완료 시 문맥교환"],
        ],
      },
      {
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["기법", "키워드", "설명"],
        rows: [
          ["BPI(우선순위 상속)", "기본 역전 해결\n시간 O(N)", "다중·재귀 불가\n공간 O(1)"],
          ["PCE(우선순위 올림)", "다중 자원 해결\nO(N*M)", "재귀 소유 불가\n공간 O(M)"],
          ["RPI(재귀적 상속)", "재귀 요청 해결\nO(N+M)", "모든 역전 해결\nO(N+M)"],
        ],
      },
    ],
    ...
    tables: [
      {
        ...
        rows: [
          ["① 호출", "read() 등\nsyscall", "응용 함수 호출\n라이브러리 실행"],
          ["② 진입", "모드 비트 변경", "커널 모드 진입"],
          ["③ 처리", "시스템 콜 번호", "함수 찾아 실행"],
          ["④ 복사", "결과 데이터", "유저 공간 복사"],
          ["⑤ 복귀", "반환 값 전달\n유저 모드 복귀", "값 두고 복귀\n다음 명령 실행"],
        ],
      },
    ...
    tables: [
      {
        ...
        headers: ["교체 기법", "키워드", "설명"],
        rows: [
          ["FIFO (First In First Out)", "최초 적재 페이지\nFIFO 큐 구현", "적재 시간 기준\n벨레이디 이상"],
          ["LRU (Least Recently Used)", "최장 미참조 교체\n참조 시간 기록", "오래 안 쓴 페이지\n막대한 오버헤드"],
          ["LFU (Least Frequently Used)", "참조 횟수 기준", "최소 참조 교체"],
          ["NUR (Not Used Recently)", "참조·수정 비트\nLRU 유사 성능", "최근 미사용 교체\n적은 오버헤드"],
          ["SCR (Second Chance Replacement)", "FIFO 보완\n큐·참조 비트", "자주 쓴 것도 대상\n비트 확인 교체"],
          ["OPT (Optimal Replacement)", "미래 최장 미사용\n부재 횟수 최소", "참조 순서 예측\n실현 가능성 희박"],
          ["MFU (Most Frequently Used)", "참조 횟수 최다", "LFU 반대 교체"],
          ["Clock Page", "SCR 동일\n원형 List", "원형 리스트 구조\n참조 비트 사용"],
        ],
      },
      {
      ...
      },
      {
        ...
        headers: ["그룹", "키워드", "설명"],
        rows: [
          ["그룹1", "참조 0·수정 0", "교체 순서 1"],
          ["그룹2", "참조 0·수정 1", "교체 순서 2"],
          ["그룹3", "참조 1·수정 0", "교체 순서 3"],
          ["그룹4", "참조 1·수정 1", "교체 순서 4"],
        ],
      },
      {
      ...
      },
      {
        ...
        rows: [
          ...
          ["문제점", "스래싱 발생\n페이지 I/O", "성능 저하 초래\n입출력 빈번"],
          ["해결방안", "부하 제어\n보류 큐 대기", "새 프로세스 지연\n생성 대기시킴"],
          ["해결방안", "지역성 활용", "시간·공간 집중"],
          ["해결방안", "워킹셋\n일정 시간 참조", "페이지 집합\n주기억장치 유지"],
          ["해결방안", "PFF 빈도", "상주 집합 조정"],
        ],
      },
    ],
    notes: [
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["Sequential Locality (순차적)", "순차적 인출 실행\n선인출 이용", "저장 순서대로\n명령어 순차 실행"],
        ],
      },
      {
        caption: "Locality 사례",
        ...
        rows: [
          ...
          ["Virtual Memory", "스레싱 해결\n워킹셋", "공간적 지역성\n페이지 교환 최소"],
          ["Virtual Memory", "NRU FIFO\nLRU 교체", "시간적·공간적\n지역성 활용"],
          ["CDN", "지역성 원리", "공간적 신속 전달"],
        ],
      },
      {
        caption: "지역성의 필요성",
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["인터럽트 서비스 루틴(ISR)", "Lock 설정", "루틴 진입 후 금지"],
          ["인터럽트 서비스 루틴(ISR)", "이전 프로세스", "문맥 정보 저장"],
          ["인터럽트 서비스 루틴(ISR)", "요청 작업 수행", "인터럽트 서비스"],
          ["인터럽트 서비스 루틴(ISR)", "이전 프로세스", "문맥 정보 복구"],
          ["인터럽트 서비스 루틴(ISR)", "상호배제 반납", "루틴 종료·허용"],
        ],
      },
      {
    ...
    tables: [
      {
        ...
        rows: [
          ["PID(프로세스 식별자)", "고유 식별자\n숫자 색인 항목", "프로세스 구분\n인덱스 활용"],
          ["프로세스 상태", "프로세스 상태\n생성 준비 실행", "상태 값 보관\n대기 중단 표시"],
          ["프로세스 상태", "상태 레지스터", "PSR 기록"],
          ["프로그램 카운터", "프로그램 카운터\n다음 명령 주소", "PC 레지스터\n실행 주소 표시"],
          ["프로그램 카운터", "Jump 처리", "분기 주소 반영"],
          ["레지스터 저장 영역", "누산기 AC\n인덱스 레지스터", "연산 결과 보관\n주소 계산 활용"],
          ["레지스터 저장 영역", "범용 레지스터\n조건 코드 정보", "AX CX 활용\n상태 플래그"],
          ["레지스터 저장 영역", "ISR MBR\nMAR 주소", "인터럽트 버퍼\n메모리 주소"],
          ["레지스터 저장 영역", "인터럽트 저장\n원상 복귀 수행", "PC와 함께 보관\n재실행 복원"],
          ["프로세서 스케줄링 정보", "우선순위 정보\n큐 포인터 보유", "실행 순서 결정\n스케줄 큐 연결"],
          ["프로세서 스케줄링 정보", "스케줄 매개변수", "정책 인자 관리"],
          ["계정 정보", "계정 정보\n프로세서 사용", "자원 사용 기록\n사용 시간 누적"],
          ["계정 정보", "실제 사용시간\n사용 상한시간", "경과 시간 측정\n한도 초과 방지"],
          ["계정 정보", "프로세스 번호", "작업 번호 기록"],
          ["입출력 상태 정보", "할당 I/O 장치\n개방 파일 목록", "장치 리스트\n열린 파일 정보"],
          ["입출력 상태 정보", "입출력 요구", "프로세스 할당"],
          ["메모리 관리 정보", "상한 하한 경계\n경계 레지스터", "영역 정의 기준\n범위 초과 방지"],
          ["메모리 관리 정보", "페이지 테이블\n세그먼트 테이블", "주소 변환 정보\n캐시 주소 보관"],
        ],
      },
  ...
  },
  {
    ...
    tables: [
      {
        caption: "멀티 쓰레드 종류",
        headers: ["Thread 종류", "키워드", "설명"],
        rows: [
          ["Single Thread", "단일 쓰레드 실행\n한 명령어 흐름", "한 번에 하나\nCPU 활용 낮음"],
          ["Interleaved Multithreading (IMT)", "시간 단위 교대\nCPU 분할 사용", "클록별 명령 선택\n하나씩 실행"],
          ["Blocked Multithreading (BMT)", "블록 시 전환\n메모리 지연 시", "블록 때만 실행\nIMT보다 낮음"],
          ["Simultaneous Multithreading (SMT)", "같은 클록 동시\n하이퍼 스레딩", "실행 유닛 공유\n활용률 극대화"],
          ["Chip Multiprocessing (CMP)", "칩 내 다수 코어\n코어별 독립", "완전 병렬 처리\n실행 유닛 독립"],
        ],
      },
    ],
    notes: [
    ...
    tables: [
      {
        ...
        headers: ["기법", "키워드", "설명"],
        rows: [
          ["FCFS(First Come First Serve)", "도착 순서 처리\n단순·공정", "요청 순서 그대로\n평균 응답 시간↑"],
          ["SSTF(Shortest Seek Time First)", "최근접 트랙 우선\n탐색 시간↓", "가까운 요청 선택\n먼 요청 기아"],
          ["SCAN(엘리베이터 알고리즘)", "한 방향 이동 처리\n응답 시간 균등", "끝 도달 후 전환\nFCFS 개선"],
          ["N-Step SCAN", "그룹별 SCAN\n신규 다음 그룹", "응답 예측 가능\n일부 요청 지연"],
          ["C-SCAN(Circular SCAN)", "원형 SCAN\n한 방향만 이동", "끝에서 처음 복귀\n복귀 시 이동 낭비"],
          ["LOOK", "SCAN 개선\n요청 범위까지만", "마지막 요청 전환\n불필요 이동 감소"],
          ["C-LOOK", "C-SCAN\n처음 요청 점프", "범위 내 한 방향\n먼 점프 시 지연"],
          ["SLTF(Shortest Latency Time First)", "회전 지연 최소\n최근접 섹터", "같은 트랙 내 선택\n여러 트랙 시 제한"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["단계", "키워드", "설명"],
        rows: [
          ["형성기(Forming)", "서로 인식 열성\n지시형 리더십", "수용·소속 형성\n팀 방향성 정립"],
          ["격동기(Storming)", "갈등 처리 관심\n지도형 리더십", "역할 명료화\n계획 역할 정리"],
          ["표준화(Norming)", "협력 관계 몰입\n참여형 리더십", "지원 자긍심 형성\n업무 역할 몰입"],
          ["수행(Performing)", "생산성 성취\n위임형 리더십", "관심 자긍심 유지\n수행 관리 평가"],
        ],
      },
    ],
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ["Withdrawal(철수/회피)", "저주장 저협력\n사소한 이슈", "관철 가능 낮음\n추가 정보 필요"],
          ["Smoothing(양보/수용)", "저주장 고협력\n상대 이슈 우선", "조화 안정 중시\n신용 확보 목적"],
          ["Compromising(타협)", "중간 주장 협력\n대등한 파워", "합의점 도출\n설득 한계 시"],
          ["Forcing(강요)", "고주장 저협력\n경쟁 우위 확보", "긴급 사안 결정\n필수 정책 집행"],
          ["Problem Solving(문제해결/대면)", "고주장 고협력\n공감대 형성", "통합 의견 도출\n관계 지속 유지"],
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    tables: [
      {
        caption: "절차 (특선상세문)",
        headers: ["절차", "입출력 산출물", "수행 내용"],
        rows: [
          ["프로젝트 특징 정의", "프로파일 작성", "문제점 수집"],
          ["표준 프로세스 선정 및 검증", "표준 프로세스\n프로세스 검증", "분석 결과 기반\n개선 영역 정의"],
          ["상위 수준의 커스터마이징", "생명주기 선정\nWBS 정의", "프로세스 특화\n부가 절차 추가"],
          ["세부 커스터마이징", "맞춤 메트릭스\n산출물 구성도", "요구사항 지원\n적합 규모 구성"],
          ["테일러링 문서화", "WBS 근거 기록", "멤버에게 공표"],
        ],
      },
      {
    ...
    notes: [
      "교재 두음: [특선상세문]",
      "세부 커스터마이징 산출물은 교재에 테일러링 메트릭스·프로젝트 WBS·산출물 구성도·프로젝트 스케쥴 네 가지다. 표에는 두 개만 들어가므로 나머지도 함께 외운다.",
      "교재 5단계의 수행 내용과 아래 7단계 절차는 「도리의 디지털라이프」 테일러링 항목에서 옮긴 것이다. 교재는 산출물 중심, 7단계는 수행 활동 중심이라 둘을 겹쳐 보면 단계마다 무엇을 하고 무엇이 나오는지가 맞춰진다.",
    ],
  },
  {
    ...
    tables: [
      {
        ...
        headers: ["구성항목", "키워드", "설명"],
        rows: [
          ["자극 유발원(Source)", "자극 생성 주체\n사람·장치", "자극 유발 존재\n외부 장치 포함"],
          ["자극(Stimulus)", "반응 유발 조건", "시스템 반응 원인"],
          ["환경(Environment)", "발생 시 상황\n정상·과부하", "자극 시점 상태\n부하 수준 구분"],
          ["대상(Artifact)", "자극 받는 대상\n시스템 범위", "자극 수용 요소\n전체 또는 일부"],
          ["응답(Response)", "도달 후 동작\n결과값 발생", "시스템 반응 동작\n측정 대상 산출"],
          ["응답 측정(Response Measure)", "측정 가능 결과\n검증 기준값", "정량 결과값 산출\n요구사항 검증"],
        ],
      },
      {
    ...
    tables: [
      {
        ...
        headers: ["구분", "키워드", "설명"],
        rows: [
          ["시나리오 기반 평가모델", "SAAM 최초\n수정성·기능성", "최초 평가 방법\n수정 가능성 집중"],
          ["시나리오 기반 평가모델", "ATAM 계승\n품질간 상충", "이전 방법 확장\n상충 관계 파악"],
          ["시나리오 기반 평가모델", "CBAM 경제성\n편익·투자가치", "비용 대비 편익\n투자가치 판단"],
          ["시나리오 기반 평가모델", "EATAM 확장\n제품라인 평가", "개별 모델 확장\n스테이지 기반"],
          ["설계/혼합 기반 평가모델", "ADR 응집도\n능동 설계 검토", "구성요소 응집도\n설계 기반 평가"],
          ["설계/혼합 기반 평가모델", "ARID 초기\n일부 설계 평가", "초기 위험 감소\n부분 설계 검토"],
        ],
      },
    ],
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["관계", "키워드", "설명"],
        rows: [
          ["연관관계(Association)", "상호 연관 존재\n다중도 표기", "두 클래스 연결\n실선으로 표기"],
          ["직접연관관계(Direct Association)", "방향성 존재\n실선 화살표", "참조 주체 구분\n방향 기호 표기"],
          ["집합연관관계(Aggregation)", "부분·전체 관계\n속 빈 마름모", "전체가 부분 포함\n약한 결합 표기"],
          ["복합연관관계(Composition)", "부분·전체 관계\n채운 마름모", "전체 소멸 시 소멸\n강한 결합 표기"],
          ["의존관계(Dependency)", "변화 영향 전달\n점선 화살표", "타 클래스 영향\n의존 방향 표기"],
          ["일반화 관계(Generalization)", "상하위 상속\nis a 관계", "공통점 상속 관계\n빈 삼각형 표기"],
          ["실체화 관계(Realization)", "인터페이스 구현\n점선 삼각형", "구현 클래스 관계\n실체화 표기"],
        ],
      },
      {
        caption: "구성 요소",
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["해법(Solution)", "패턴 구성요소\n요소 역할 관계", "구성 단위 정의\n캡슐화·위임"],
          ["해법(Solution)", "인터페이스\n구성과 위임", "요소 간 관계\n협력 방법 제시"],
          ["해법(Solution)", "구조 다이어그램\n구현·샘플코드", "클래스 도식화\n코드 예시 제공"],
          ["결과(Consequence)", "적용 결과 서술\n효과·주의사항", "장단점 기술\n활용 시 유의"],
          ["결과(Consequence)", "활용사례\n관련패턴", "적용 예시 제시\n연관 패턴 안내"],
        ],
      },
      {
        caption: "디자인 패턴 분류 [생구행]",
        headers: ["패턴 분류", "키워드", "설명"],
        rows: [
          ["생성패턴(Creational)", "객체 생성 방식\n정의·캡슐화", "생성 구조화\n클래스 정의"],
          ["생성패턴(Creational)", "팩토리메소드", "클래스 범위"],
          ["생성패턴(Creational)", "추상팩토리\n빌더 패턴", "객체 범위 적용\n단계적 생성"],
          ["생성패턴(Creational)", "프로토타입\n싱글턴 패턴", "복제로 생성\n단일 인스턴스"],
          ["구조패턴(Structural)", "객체 조직화\n라이브러리", "구성 방식 제시\n통합 시 유용"],
          ["구조패턴(Structural)", "어댑터·브리지", "변환·분리 구조"],
          ["구조패턴(Structural)", "컴포지트\n데코레이터", "복합 구조화\n기능 동적 추가"],
          ["구조패턴(Structural)", "퍼사드·프록시", "단순 창구 대리"],
          ["구조패턴(Structural)", "플라이웨이트", "객체 공유 절감"],
          ["행위패턴(Behavioral)", "객체 행위 관리\n연동 유형", "행위 조직 연합\n클래스 연동"],
          ["행위패턴(Behavioral)", "인터프리터\n템플릿메소드", "문법 표현 해석\n알고리즘 골격"],
          ["행위패턴(Behavioral)", "책임 연쇄\n커맨드 패턴", "요청 전달 처리\n요청 캡슐화"],
          ["행위패턴(Behavioral)", "미디에이터\n메멘토 패턴", "중재자 통신\n상태 저장 복원"],
          ["행위패턴(Behavioral)", "이터레이터\n스테이트", "순차 접근 제공\n상태별 행위"],
          ["행위패턴(Behavioral)", "스트래티지\n옵서버 패턴", "알고리즘 교체\n상태 변경 통지"],
          ["행위패턴(Behavioral)", "비지터 패턴", "연산 분리 적용"],
        ],
      },
    ],
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["CSF", "키워드", "설명"],
        rows: [
          ["Silo 조직의 통합", "정보·책임 공유", "SRE 개발 공유"],
          ["Silo 조직의 통합", "동일 개발 도구\n의사소통 도구", "공통 도구 사용\n소통 채널 통일"],
          ["Silo 조직의 통합", "오너십 공유", "책임 함께 부담"],
          ["실패에 대한 수용", "위험 감수\n포스트모템", "실패 허용 문화\n장애 사후 회고"],
          ["실패에 대한 수용", "오류 정량화\nError 예산", "가용성 수치화\n오류 허용 관리"],
          ["점진적 구현과 개선", "카나리 배포\n롤링 업데이트", "점진 노출 확대\n무중단 갱신"],
          ["점진적 구현과 개선", "실패 비용 절감", "올바른 방향 지원"],
          ["자동화 시스템 사용", "일상 반복 업무\n업무 자동화", "수작업 제거\n오류 최소화"],
          ["자동화 시스템 사용", "Toil 관리", "운영 부담 축소"],
          ["모든 것에 대한 측정", "측정 방법 정의\n시스템 지표", "정량 기준 수립\n작동 상태 관측"],
          ["모든 것에 대한 측정", "수동 작업 시간\n장애 시간", "운영 부하 측정\n가용성 산정"],
        ],
      },
    ],
  },
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["불완전", "완벽 테스트 불가\n자원의 한계", "전수 검증 불가능\n시간·비용 제약"],
          ["초기 시작", "설계 시 고려\n결함 조기 발견", "개발 초기 착수\n품질 비용 감소"],
          ["결함 집중(Defect Clustering)", "20% 모듈 집중\n파레토 법칙", "결함 80% 편중\n특정 영역 집중"],
          ["살충제 패러독스(Pesticide Paradox)", "동일 케이스 반복\n결함 미발견", "반복 수행 한계\n케이스 맞춤 수정"],
          ["정황 의존적", "주변 환경 영향\n도메인 의존", "정황 따라 달라짐\n분야별 목표 상이"],
          ["오류 부재의 궤변(Absence-errors fallacy)", "요구 미충족\n결함 제거 무의미", "비즈 요구 충족\n제거해도 무용"],
        ],
      },
      {
    ...
    tables: [
      {
        ...
        rows: [
          ["① 계획활동", "인원·역할 배분\n리뷰 대상 선정", "참가자 역할 분배\n시작·종료 기준"],
          ["② 시작(Kick-Off)", "문서 배포 실시\n목표·절차 설명", "리뷰 개요 공유\n시작 기준 점검"],
          ["③ 사전 검토/개별 준비", "개별 사전 검토\n잠재 결함 기록", "참석자별 준비\n질문·의견 정리"],
          ["④ 리뷰 미팅", "결함 여부 결정\n처리 방안 제안", "토의 결과 기록\n상세 의견록 작성"],
          ["⑤ Re-Work(재작업)", "발견 결함 수정\n재작업 수행", "지적 사항 반영\n추가 활동 없음"],
          ["⑥ Follow-Up(후속처리 확인)", "결함 조치 확인\n측정치 수집", "후속 처리 점검\n종료 기준 확인"],
        ],
      },
      {
        caption: "리뷰의 형식",
    ...
    tables: [
      {
        ...
        headers: ["구분", "키워드", "설명"],
        rows: [
          ["텍스트(Text)", "텍스트 분류", "단일·다중 라벨"],
          ["텍스트(Text)", "개체명 인식", "단어·구문 라벨"],
          ["텍스트(Text)", "관계 의존성 정의", "단어 라벨링 관계"],
          ["이미지(Image)", "이미지 분류", "단일·다중 라벨"],
          ["이미지(Image)", "객체 인식\n사각·다각 영역", "바운딩박스\n폴리곤 표기"],
          ["비디오(Video)", "동영상 분류", "단일·다중 라벨"],
          ["비디오(Video)", "영상 객체 인식\n정점 위치 표시", "바운딩박스\n키포인트 정점"],
          ["비디오(Video)", "영상 객체 추적\n프레임 간 경로", "폴리곤 다각형\n폴리라인 선"],
          ["오디오(Audio)", "오디오 분류", "클래스 라벨"],
          ["오디오(Audio)", "세그먼테이션\n음성 인식 STT", "오디오 구간 분할\n텍스트 전사"],
          ["기타", "시계열 분할\nHTML 분류", "구간 클래스 라벨\n문서 클래스 라벨"],
        ],
      },
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["최적화", "경사하강법 조정", "가중치 업데이트"],
          ["최적화", "가중치 업데이트", "반복 성능 향상"],
        ],
      },
      {
        caption: "학습 단계",
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["혼동행렬 환산수식", "실제 양성 검출", "TP/실제 양성"],
          ["혼동행렬 환산수식", "정밀·재현 조화", "조화 평균 계산"],
          ["임계값 변화", "X축 위양성률\nY축 진양성률", "임계값 변화 곡선\n수신자 조작 특성"],
          ["임계값 변화", "ROC 아래 면적", "1 근접 시 우수"],
        ],
      },
      {
        caption: "거리 (유맨민체코)",
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["외부평가", "정답 기반 평가", "정답지 정확도"],
          ["Dunn Index", "군집 간 거리 기반\n내부 분산 작음", "최소 최대 거리비\n분리 클수록 우수"],
          ["Elbow Method", "중심 거리 기반", "적정 K 최소값"],
        ],
      },
      {
        caption: "수행절차",
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ["선형", "PCA 주성분", "무상관 저차원화"],
          ["선형", "LDA 분산비", "비율 최대화 축소"],
          ["선형", "SVD 행렬 분해", "임의 행렬 분해"],
          ["선형", "변수 관계 분석", "공통 차원 축약"],
          ["비선형", "ISOMAP\nMDS+PCA", "비선형 축소\n두 방법 결합 확장"],
          ["비선형", "로컬 선형 임베딩\n인접 선형 구조", "LLE 저차원화\n구조 보존 임베딩"],
          ["비선형", "압축 후 복원", "신경망 기반 축소"],
          ["비선형", "저차원 격자 대응", "신경망 군집 도출"],
        ],
      },
    ],
  },
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["선택", "임의 개체 추출", "최고 적합도 선택"],
          ["교차", "특정 지점 기준", "유전자 교차 진행"],
          ["교차", "유전자별 난수", "난수값 따라 선택"],
          ["교차", "산술 연산 적용", "현재 유전자 연산"],
        ],
      },
    ],
    ...
    tables: [
      {
        ...
        rows: [
          ["코사인 유사도", "cosθ 내적\n벡터 방향성", "코사인 각도 계산\n0은 일치점 없음"],
          ["해밍 거리", "상이 비트 수\n동일 길이 XOR", "이진 벡터 차이 수\n길이 다르면 불가"],
          ["자카드 인덱스", "교집합/합집합\n비율 0~1", "집합 관계 유사도\n1 근접 시 동일"],
          ["소렌슨-다이스 인덱스", "교집합 2배 반영\n공동/평균 원소", "일치 정도 강조\n원소 비율 유사도"],
        ],
      },
    ],
  },
    ...
    tables: [
      {
        ...
        headers: ["거리", "키워드", "설명"],
        rows: [
          ["유클리디안 거리(L2, 직선)", "제곱합 제곱근", "두 점 직선거리"],
          ["맨하탄 거리(L1, 직각)", "Σ|a−b|", "수평·수직 이동"],
          ["체비쇼프 거리", "좌표차 최대값", "가장 긴 차원 거리"],
          ["마할라노비스 거리(상관관계)", "상관·분산 고려\n이상치 탐지", "통계적 거리 측정\n평균 멀수록 이상"],
          ["민코프스키 거리(복합)", "p 제곱합 일반화\np=1,2,∞", "세 거리의 일반화\n각 p별 거리 대응"],
        ],
      },
    ],
  },
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["Latent Space", "평균·표준편차", "입력 분포 학습값"],
          ["Latent Space", "사후 확률 추론\n변분 추론 근사", "평균·편차 이용\n근사적 학습"],
          ["디코더(Decoder)", "사후 확률 최대화", "분포 학습 출력값"],
          ["디코더(Decoder)", "신규 데이터 생성", "입력과 유사 출력"],
        ],
      },
      {
        caption: "목적",
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["학습모델", "용도별 미세 조정", "후처리 조정 과정"],
          ["프레임워크", "기능 연결·통합", "에이전트·콜백"],
          ["프레임워크", "벡터 임베딩 저장", "유사도 인덱싱"],
          ["프레임워크", "프롬프트 설계", "원하는 결과 도출"],
        ],
      },
      {
        caption: "문제점 및 대응방안",
    ...
    tables: [
      {
        ...
        headers: ["구분", "키워드", "설명"],
        rows: [
          ["데이터처리", "범용 데이터 학습", "이미 훈련된 모델"],
          ["인터렉션", "특정 작업 도메인", "튜닝 대상 데이터"],
          ["인터렉션", "타깃 데이터 학습", "사전 모델에 훈련"],
          ["인터렉션", "튜닝 모델 검증", "성능 검증 단계"],
        ],
      },
      {
        caption: "파인튜닝 방법",
  ...
  },
  {
    ...
    tables: [
      {
        caption: "주요기법 [추R모효멀]",
        headers: ["기술 그룹", "키워드", "설명"],
        rows: [
          ["추론 능력 강화(Reasoning Enhancement)", "CoT·ToT\nLtM 기법", "중간 추론 생성\n문제 분해 해결"],
          ["외부 지식 활용 및 정밀 검색(Retrieval-Augmented Generation, RAG)", "RAG 검색\n외부 DB 참조", "최신 정보 반영\n문맥 보강 응답"],
          ["모델 병합 및 결합(Merging & Integration)", "가중치 보간\nDARE 병합", "사전학습 결합\n태스크 맞춤화"],
          ["효율성 및 비용 절감(Optimization & Efficiency)", "MoE·희소화\nLoRA 양자화", "연산 비용 절감\n경량 성능 유지"],
          ["멀티모달 통합(Multimodal Integration)", "CLIP 계열\n영상·음성", "이미지 결합 처리\n모달 확장 처리"],
        ],
      },
      {
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ["Sampling 기반", "N개 샘플링\n신뢰도 기반", "다수 응답 생성\n최고 신뢰 선택"],
          ["Decoding 기반", "빔서치 확장", "중간 평가 확장"],
          ["Decoding 기반", "자기 일관성", "다수결 결정"],
          ["Reasoning 기반", "CoT 사고사슬", "단계 명시 유도"],
          ["Reasoning 기반", "ToT 트리", "분기 탐색 추론"],
          ["Reasoning 기반", "GoT 그래프", "유연 연결 추론"],
          ["Search & Verification 기반", "검증기 탐색", "보상모델 평가"],
          ["Search & Verification 기반", "MCTS 탐색", "경로 확장 평가"],
          ["Self-Improvement 기반", "자기 수정 반복", "비판·수정 순환"],
          ["Compute 최적화 기반", "난이도 판단", "순차·병렬 배분"],
        ],
      },
      {
        ...
        rows: [
          ["목적", "모델 능력 확장", "추론 성능 향상"],
          ["비용 구조", "초기 비용 큼", "유연 비용 할당"],
          ["변경 여부", "파라미터 수정", "파라미터 유지"],
          ["강점", "새 능력 획득", "품질·정확 향상"],
          ["단점", "재훈련 비용", "속도 지연 우려"],
          ["대표 사례", "GPT-4 학습", "MCTS 활용"],
        ],
      },
    ],
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["변환 기반", "주파수 변환", "도메인 삽입"],
          ["학습 기반", "생성모델 학습", "서명 은닉 생성"],
          ["학습 기반", "텍스트 생성", "문장 중 삽입"],
        ],
      },
      {
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["설계 원칙", "HTTP SSE\nJSON RPC", "기존 웹 표준\n원격 호출 표준"],
          ["설계 원칙", "OpenAPI\n동등 수준 인증", "인증체계 기준\n엔터프라이즈급"],
          ["설계 원칙", "실시간 피드백", "알림·상태 갱신"],
          ["주요 기능", "오디오·비디오", "스트리밍 지원"],
          ["주요 기능", "JSON 카드", "자기 기능 공유"],
          ["주요 기능", "에이전트 간 통신", "작업 완료 지향"],
          ["주요 기능", "컨텍스트·답변", "지시사항 전달"],
          ["주요 기능", "파트 콘텐츠 유형", "UI 기능 명시"],
        ],
      },
    ],
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["MCP Server 측면", "접근 권한 축소\n필요 범위 한정", "파일 접근 제한\n시스템 접근 통제"],
          ["MCP Client 측면", "주기적 변경 검사\n악성코드 탐지", "변경 여부 확인\n비인가 수정 발견"],
          ["MCP Client 측면", "사용 기록 보관\n이상 징후 감지", "감사 체계 유지\n비정상 행동 포착"],
          ["MCP Client 측면", "MCP 위험 교육\n안전 사용법", "내부 사용자 대상\n운영자 숙지"],
        ],
      },
    ],
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["2. 서비스 이용료 계산", "도입기간 규모\n단위 총 이용료", "사용 규모 반영\n이용료 산출물"],
          ["3. 커스터마이징 작업비용 계산", "비용항목 식별\n유사 서비스 견적", "작업 범위 확정\n가격표 기준 산정"],
          ["3. 커스터마이징 작업비용 계산", "요구분석 설계\n데이터 구축", "요건 정의 반영\n수집 정제 가공"],
          ["3. 커스터마이징 작업비용 계산", "모델 구현 학습\n검증 안정화", "학습 수행 비용\n검수 후 안정화"],
          ["4. 구축·개발 비용 계산", "SW 개발 통합\n기능점수 공수", "작업 항목 식별\n산정 방식 적용"],
          ["5. AI 서비스 도입 사업비 산정", "이용료 맞춤비\n구축 개발비", "비용 항목 합산\n도입 사업비 산출"],
        ],
      },
    ],
  ...
  },
  {
    ...
    tables: [
      {
        caption: "연관성 분석 유형",
        headers: ["분석 방법", "키워드", "설명"],
        rows: [
          ["교차분석", "명목척도 자료\n교차분할표", "범주형 간 관계\n독립성 분석"],
          ["스피어만 서열 상관분석", "서열척도 변수\n선형 상관분석", "서열 변수 간 관계\n상관 관계 파악"],
          ["피어슨 상관분석", "등간·비율척도\n연속형 변수", "두 변수 선형 관계\n상관분석 통계"],
          ["편상관분석", "제3 변수 통제\n등간·비율척도", "통제 후 두 변수\n상관관계 분석"],
        ],
      },
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["유형", "키워드", "설명"],
        rows: [
          ["One Way ANOVA", "독립1·종속1\n단일변량 분석", "한 집단 구분 변수\n급여→생산성"],
          ["Repeated Measures ANOVA", "독립1·종속1\n반복 측정 분석", "집단 반복 측정\n1·3·6개월"],
          ["Two Way ANOVA", "독립2·종속1\n단일변량 분석", "두 집단 구분 변수\n급여+나이 예"],
          ["Multi Way ANOVA", "독립3+ 종속1\n단일변량 분석", "다수 구분 변수\n급여·나이 등"],
          ["Multivariate ANOVA", "독립1 종속2+\n다변량 분산분석", "한 집단 구분 변수\n생산성+만족도"],
        ],
      },
    ],
  },
    ...
    tables: [
      {
        ...
        headers: ["종류", "키워드", "설명"],
        rows: [
          ["Reset All 기법", "축적 TC 전부\n고위험 분야", "전량 재테스트\n금융·고객 업무"],
          ["Selective 기법", "변경 대상 위주\n일반 기업 시스템", "영향 범위 결정 후\n선별 테스트 수행"],
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["원리", "키워드", "설명"],
        rows: [
          ["계속적 변경(Continuing Change)", "현실 반영 변경\n유용성 저하", "요구 따라 변경\n미변경 시 저하"],
          ["복잡도 증가(Increasing Complexity)", "변경 시 복잡화\n유용성 유지", "구조 복잡도 증가\n변경 반복 요인"],
          ["프로그램 진화(Program Evolution)", "고유 크기 유지\n에러 수 일정", "릴리즈 간격 일정\n변경 후도 동일"],
          ["조직적 안정화(Organizational Stability)", "생산성 일정\n자원 상한선", "생명주기 내 유지\n추가 투입 무의미"],
          ["친근성 유지(Conservation of Familiarity)", "버전 변화 일정\n사용성 유지", "급격 변화 없음\n일정 차이 변화"],
          ["지속적 성장(Continuing Growth)", "기능 지속 추가", "사용자 만족 확보"],
          ["품질 감소(Declining Quality)", "운영 환경 변화\n미대응 시 저하", "환경 변화 발생\n품질 수준 감소"],
          ["피드백 시스템(Feedback System)", "환경 반영 구조\n큰 폭 제품개선", "피드백 체계 구성\n개선 반영 순환"],
        ],
      },
    ],
  },
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["정책수립", "인력·역할 구성", "R&R 운영 방안"],
          ["획득", "고객 요구 분석", "고민·요구 파악"],
          ["획득", "적합 OSS 탐색", "신규 후보 조사"],
          ["획득", "OSS 속성 구분", "상태·수준 정리"],
          ["획득", "가중치 부여", "평가 모델 채점"],
          ["적용", "책임·의무 조건", "도입·배포 규제"],
          ["적용", "기능·사양 구성", "요구 분석 반영"],
          ["적용", "변경 및 결합", "OSS 프로그램"],
          ["적용", "단일 프로그램", "설치 편의 제공"],
          ["적용", "품질·성능 확인", "요구 수준 검증"],
          ["적용", "매체·웹·장비", "사용자 전달"],
          ["운영 및 유지", "운영 장비 탑재", "실행 환경 구성"],
          ["운영 및 유지", "지속 실행 가동", "서비스 유지"],
          ["운영 및 유지", "제반 작업 수행", "최상 상태 유지"],
          ["운영 및 유지", "문제 해결 지원", "추가 요구 반영"],
          ["운영 및 유지", "소스코드 기여\n재정·교류 참여", "코드 기여 활동\n후원·참여 제시"],
          ["관리 및 개선", "라이선스 준수", "법적 문제 해결"],
          ["관리 및 개선", "도입 활용 배포\n역량 강화", "OSS 지식 전달\n교육 통한 강화"],
          ["관리 및 개선", "적용 후 점검", "피드백 수렴"],
        ],
      },
    ],
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["프로젝트 관리", "의사소통 조율", "업체 업무 관리"],
          ["개발 영역", "수집·분석 체계", "요구사항 관리"],
          ["개발 영역", "요구 기반 설계", "시스템 구현 수행"],
          ["개발 영역", "기능·성능 검증", "테스트 수행"],
          ["지원 영역", "프로세스 보증", "산출물 품질 확보"],
          ["지원 영역", "변경·버전 관리", "산출물 체계 관리"],
          ["지원 영역", "정량적 측정", "성과 분석 수행"],
          ["조직관리 영역", "프로세스 정의", "운영·관리 체계"],
          ["조직관리 영역", "역량 강화 교육", "교육 체계 구축"],
          ["프로세스 개선영역", "성과 관리·평가", "조직 성과 체계"],
          ["프로세스 개선영역", "문제 원인 분석", "해결 방안 실행"],
          ["프로세스 개선영역", "지속적 개선", "프로세스 최적화"],
        ],
      },
      {
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["Step 3", "FT 다이어그램", "결함 트리 도식화"],
          ["Step 4", "정성적 구조 분석", "기본 사상 파악"],
          ["Step 5", "FT 정량화\n고장율·에러", "발생확률 조사\n빈도 데이터 정리"],
          ["Step 6", "해석 결과 평가", "위험 수준·대책"],
        ],
      },
      {
        caption: "표기법",
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["구분", "키워드", "설명"],
        rows: [
          ["프로젝트 요소", "난이도·유형", "개발언어 고려"],
          ["프로젝트 요소", "트랜잭션 규모", "입출력·연계 수"],
          ["프로젝트 요소", "정확성·견고성", "완전·일관 수준"],
          ["자원 요소", "관리자·개발자", "지원체계 포함"],
          ["자원 요소", "개발·운영 장비", "장비 확보 수준"],
          ["자원 요소", "개발지원 도구", "테스트 툴 확보"],
          ["생산성 요소", "경험·전문지식", "습득 정도 반영"],
          ["생산성 요소", "업무 요건 지식", "고객 이해 수준"],
          ["생산성 요소", "최신 기법 적용", "개발·관리 기법"],
        ],
      },
      {
        caption: "규모산정 방법 비교",
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ["사전준비", "기능 요구사항\n정통법·간이법", "개발 업무 정의\n규모산정법 결정"],
          ["개발대상 SW 기능점수 산정", "기능 식별 복잡도", "기능점수 산정"],
          ["보정 전 개발원가 산정", "기능점수×단가", "보정 전 원가 산정"],
          ["보정 후 개발원가 산정", "규모 연계 성능\n환경·보안 보정", "보정요소별 계수\n원가×보정계수"],
          ["직접경비 및 이윤 산정", "관련 직접경비\n이윤 25% 이내", "개발 관련 경비\n개발원가 기준"],
          ["소프트웨어 개발비 산정", "개발원가+경비\n이윤 가산 합계", "개발비 총액 산정\n최종 대가 확정"],
        ],
      },
    ],
    ...
    tables: [
      {
        ...
        rows: [
          ["정보화 사업", "대국민 서비스\n공동 구축 사용", "행정 민원 업무\n여러 중앙기관"],
          ["정보화 사업", "5억 원 이상", "사업비 기준 초과"],
          ["정보화 사업", "기관장 인정", "감리 필요성 판단"],
          ["행정/공공 기관", "중앙행정기관\n소속기관 지자체", "국가 행정 조직\n산하 지방 조직"],
          ["행정/공공 기관", "공운법 4조 법인\n단체 또는 기관", "공공기관 운영법\n법정 지정 대상"],
          ["행정/공공 기관", "지방공사 공단\n특별법 특수법인", "지방공기업법상\n설립 근거 보유"],
          ["행정/공공 기관", "초중등교육법\n고등교육법 학교", "각급 학교 포함\n대학 등 해당"],
          ["행정/공공 기관", "대통령령 기관", "그 밖 지정 법인"],
        ],
      },
      {
      ...
      },
      {
        ...
        headers: ["단계", "키워드", "설명"],
        rows: [
          ["요구정의단계", "A00 예비조사", "감리계획서 작성"],
          ["설계단계", "B00 현장감리", "수행결과보고서"],
          ["종료단계", "C00 조치확인", "조치확인보고서"],
        ],
      },
    ],
  },
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["구조", "최하위 동일 레벨", "리프 높이 동일"],
          ["특징", "m/2 이상 자식\n키 개수 하한", "리프·루트 제외\nm/2−1 이상"],
          ["특징", "최악 경우 없음\nO(logN)", "균등한 탐색 시간\n시간 복잡도 일정"],
          ["특징", "많은 자식 노드", "트리 높이 감소"],
          ["특징", "노드 내 정렬", "자료 정렬 저장"],
          ["유형", "인덱스 세트\n순차 세트", "색인 부분 구성\n순서 부분 구성"],
          ["유형", "2/3 이상 채움", "루트·리프 제외"],
        ],
      },
      {
        caption: "삽입/삭제 연산",
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["Composite Key Table", "복합 키 인덱스", "복합 인덱스 구성"],
        ],
      },
      {
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["Soft State", "노드 간 데이터\n외부 정보 결정", "내부 정보 무관\n도달 시점 갱신"],
          ["Eventually Consistent", "일시적 비일관성\n도착 후 일관성", "도달 전 불일치\n최종 일관성 회복"],
        ],
      },
    ],
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ["① 벡터 임베딩", "텍스트·이미지\n고차원 벡터", "원본 데이터 입력\n숫자 벡터 변환"],
          ["② 데이터 저장 및 인덱싱", "해싱 기반\n해시 버킷", "유사 벡터 매핑\n동일 버킷 할당"],
          ["② 데이터 저장 및 인덱싱", "하위 벡터 분할\n공간별 양자화", "고차원→저차원\n벡터 압축 저장"],
          ["② 데이터 저장 및 인덱싱", "벡터→노드\n유사도→엣지", "그래프 구성\n벡터 관계 표현"],
          ["② 데이터 저장 및 인덱싱", "트리 기반 인덱스\n계층적 분할", "벡터 공간 분할\n검색범위 축소"],
          ["③ 쿼리 처리", "검색 데이터 입력\n동일 벡터화 모델", "원본 데이터 형태\n쿼리 벡터 변환"],
          ["④ 유사성 측정", "코사인·내적\n유클리드 등", "각도·성분곱 합\n직선·직각 거리"],
          ["⑤ 후처리", "추가 필터링\n순위 재조정", "초기 결과 정제\n최종 결과 확정"],
        ],
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["① 탐색", "공통요소 탐색", "변환·범위 설정"],
          ["① 탐색", "이상값 처리", "유형별 관계파악"],
          ["② 분석", "관계 명확 규명", "형태 의미 발견"],
          ["② 분석", "2차 탐색·목표", "시각디자인 표현"],
          ["② 분석", "추세선·회귀선", "분석 시각적 표현"],
          ["② 분석", "지표 기본구조\n관계 축약 표현", "지표 설정 고려\n인과 데이터 검토"],
          ["③ 활용", "내·외부 활용", "가치 검증 단계"],
          ["③ 활용", "구체적 탐색 발전", "모델 서비스 적용"],
          ["③ 활용", "설득용 시각화", "소통 디자인 활용"],
          ["③ 활용", "종합 오류 검토\n상하 관점 지표", "오류 보완 수행\n실시간 지표 검토"],
        ],
      },
    ],
    ...
    tables: [
      {
        ...
        headers: ["구분", "대상", "산출물"],
        rows: [
          ["재해복구(Disaster Recovery)", "핵심업무 App", "재해복구 계획"],
          ["업무복구(Business Recovery)", "핵심 프로세스", "업무복구 계획"],
          ["업무재개(Business Recovery Resumption)", "프로세스 전반", "대체 절차 계획"],
          ["비상계획(Contingency Plan)", "내·외부 사건", "업무 비상 계획"],
        ],
      },
      {
    ...
    tables: [
      {
        ...
        rows: [
          ["Star(육성 사업)", "고점유·고성장\n지속 투자 육성", "수익 흐름 양호\n성장 동력 확보"],
          ["Cash Cow(합리화 사업)", "고점유·저성장\n자금 재배치", "낮은 투자 고수익\n수요 정체 대응"],
          ["Question Mark(신규 사업)", "저점유·고성장\n선별적 투자", "수익 낮은 단계\n성장 기회 존재"],
          ["Dog(철수 대상)", "저점유·저성장\n수익성 저조", "한계 사업 판단\n퇴출 검토 대상"],
        ],
      },
      {
    ...
    tables: [
      {
        ...
        rows: [
          ["산업재산권", "특허 기술사상\n고도한 발명", "자연법칙 창작\n수준 높은 창작"],
          ["산업재산권", "물품 형상·구조", "기술적 사상 창작"],
          ["산업재산권", "형상 모양 색채", "시각적 미감 유발"],
          ["산업재산권", "상품 식별 표장", "상품·포장 표시"],
          ["저작권", "경제적 이익 향수\n재산적 권리", "복제 공연 방송\n전시 배포권"],
          ["저작권", "저작권자 인격\n공표·이름표시", "주체 분리 불가\n동일성 유지권"],
          ["저작권", "실연 음반 방송", "인접 사업자 권리"],
          ["신지식재산권", "경쟁우위 비밀\n침해 시 구제", "비용·자원 투입\n영업비밀보호법"],
          ["신지식재산권", "DB 구축 투자", "투자 인식 권리"],
          ["신지식재산권", "식물발명 특허\n식물품종 보호", "특허법 적용\n종자산업법 적용"],
          ["신지식재산권", "배치설계 법률\n특허법 29조", "집적회로 배치법\n관련 법률 근거"],
        ],
      },
    ...
    tables: [
      {
        ...
        headers: ["구분", "세부", "설명"],
        rows: [
          ["원칙", "주요 과제 집중", "구성원 정확 소통"],
          ["원칙", "개인·기업 목표\n투명 공개·공유", "목표 상호 연결\n구성원 목표 공개"],
          ["원칙", "상황 추적 관리\n문제 발생 시", "현재 상태 파악\n해결방안 모색"],
          ["원칙", "자신의 한계 시험\n실패 자유 부여", "도전 반복 수행\n실패·수정 반복"],
          ["항목", "조직 미션 달성", "스스로 설정 목표"],
          ["항목", "목표 달성 판단", "핵심 지표 정의"],
          ["프로세스", "직원 공감 확보\n기간 목표 KR", "OKR 공감 형성\n진행 범위 설정"],
          ["프로세스", "스코어 포인트\n3단계 상태", "직관적 상태 측정\n진행 상태 표시"],
          ["프로세스", "1:1 미팅 결과", "효율적 개선 반영"],
        ],
      },
      {
        caption: "주요 미팅 및 원칙",
      ...
      },
      {
        ...
        headers: ["항목", "OKR", "MBO"],
        rows: [
          ["시작 시기", "70년대 인텔", "50년대 드러커"],
          ["주요 지표", "KR 핵심 결과\nOKR 결과 지표", "KPIs 지표\nSMART 원칙"],
          ["목표 특성", "정성적 문샷", "정량적 루프샷"],
          ["적용 방식", "상향식 수립", "하향식 수립"],
          ["성과 관리", "간접적 연계", "직접적 연계"],
        ],
      },
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["해시 출력 길이", "출력 비트 수\n공격 저항성", "길이 비례 강도\n충돌·역상 저항"],
          ["운용 모드", "운용 모드 방식\n모드별 상이", "평문 노출 여부\n무결성 여부"],
          ["초기화 벡터(IV)·논스(Nonce) 사용 방식", "무작위성 확보\n유일성 보장", "예측 불가 필요\n재사용 시 취약"],
        ],
      },
      {
    ...
    tables: [
      {
        ...
        rows: [
          ["구축·운용 시", "5만명 민감정보\n고유식별정보", "구축·변경 파일\n처리 사업 대상"],
          ["구축·운용 시", "50만명 연계", "공공기관 내외부"],
          ["구축·운용 시", "100만명 이상", "정보주체 파일"],
          ["변경 시", "심사 후 체계변경", "변경 부분 재평가"],
        ],
      },
      {
        caption: "PIA 수행 절차",
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["M.2", "은행 입금 알림", "수신 정보 확인"],
          ["M.3", "paid 상태", "결제 정보 저장"],
          ["M.4", "발행 Mint\n대기열 추가", "토큰 발행 시작\n작업 큐 등록"],
          ["M.5", "대기열 작업 수신", "실제 발행 수행"],
          ["M.6", "TX 생성·서명", "온체인 전파"],
          ["M.7", "계약 상호작용", "처리 여부 확인"],
          ["M.8", "민팅 결과 저장", "참조 감사용 기록"],
        ],
      },
      {
        caption: "소각절차(Burn)",
        ...
        rows: [
          ...
          ["B.2", "오프체인 서명\nPermit", "사용자 승인 부여\n소각 권한 허용"],
          ["B.3", "소각 함수 호출", "인터페이스 실행"],
          ["B.4", "TX 모니터링", "성공 여부 확인"],
          ["B.6", "BaaS 송금", "사용자 계좌 이체"],
          ["B.7", "소각 송금 이벤트\n영구 기록", "저장소 기록\n무결성 확보"],
        ],
      },
    ],
    ...
    tables: [
      {
        ...
        rows: [
          ["1단계(자율적 의사결정)", "목표 조작·변경", "비인가 목표 설정"],
          ["1단계(자율적 의사결정)", "허위 응답 생성\n비허용 방식 사용", "의도적 오작동\n기만적 목표 달성"],
          ["1단계(자율적 의사결정)", "작업 기록 누락\n기록 조작 가능", "사후 분석 불가\n책임 추적 불가"],
          ["2단계(메모리 활용)", "허위 정보 삽입", "잘못된 학습 유도"],
          ["2단계(메모리 활용)", "부정확 데이터", "잘못된 의사결정"],
          ["3단계(외부 도구 및 시스템 호출)", "도구에 악성코드", "악의적 행동 유도"],
          ["3단계(외부 도구 및 시스템 호출)", "에이전트 권한", "비인가 작업 수행"],
          ["3단계(외부 도구 및 시스템 호출)", "시스템 자원 고갈", "성능 저하 유발"],
          ["3단계(외부 도구 및 시스템 호출)", "악성 원격 명령", "실행 수행 유도"],
          ["4단계(인증 및 권한 관리)", "인증 체계 우회\n사용자·AI", "사칭 비인가 작업\n타 개체로 위장"],
          ["5단계(인간 개입)", "과다 작업 처리\n인간 개입 방해", "검토 불가 수준\n공격자 유도"],
          ["5단계(인간 개입)", "에이전트 조작\n보안 정책 무력화", "인간 오도 유도\n잘못된 행동 유도"],
          ["6단계(다중 에이전트)", "에이전트 간 통신\n도구 간 통신 조작", "통신 내용 조작\n잘못된 정보 확산"],
          ["6단계(다중 에이전트)", "신뢰 체계 악용", "비인가 작업 수행"],
