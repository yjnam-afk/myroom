    ...
    tables: [
      {
        ...
        headers: ["번호", "역할", "설명"],
        rows: [
          ["①", "프로세스 관리 및 CPU Scheduling", "스케줄러·IPC·동기화로 프로세스 생성·실행 순서를 관리"],
          ["②", "메모리 관리", "가상 메모리 등 프로세스에 메모리를 할당·회수"],
          ["③", "I/O Device 관리", "장치 드라이버(Device Drivers)로 입출력 장치를 제어"],
          ["④", "IPC(Inter Process Communication)", "프로세스 간 데이터 교환·통신 제공"],
          ["⑤", "네트워크", "네트워크 프로토콜 처리와 통신 지원"],
          ["⑥", "File System", "VFS·파일 시스템으로 파일 저장·접근 관리"],
        ],
      },
      {
        caption: "커널의 구조(Architecture)",
        headers: ["계층", "구성 요소", "설명"],
        rows: [
          ["System Call Interface", "시스템 호출 인터페이스", "사용자 프로그램이 커널 기능을 요청하는 진입점"],
          ["관리 계층", "파일 시스템 관리 / I/O 관리(Device Drivers) / 메모리 관리", "자원 종류별 관리 모듈이 서로 연계"],
          ["관리 계층", "프로세스 관리(스케줄러·IPC·동기화)", "프로세스 실행·통신·동기화를 담당"],
          ["Protection", "보호", "커널 자원에 대한 접근을 통제"],
          ["Hardware Control", "Interrupt Handler 등", "인터럽트 처리로 하드웨어를 직접 제어"],
          ["Hardware", "CPU·메모리·장치", "커널이 관리하는 물리 자원"],
        ],
      },
      {
        caption: "커널의 종류 — 모놀리틱 커널과 마이크로 커널의 비교",
