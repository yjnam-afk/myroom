...
export type TextbookSubnote = {
  ...
  course: "CA" | "OS" | "PM" | "SE" | "AI" | "ST" | "DS" | "AL";
  /** 교재 '■ 정의' 그대로 */
  definition: string;
...
export const SUBNOTES: TextbookSubnote[] = [
  ...
  },
  {
    ...
    course: "AL",
    definition:
      "인접한 두 원소를 차례로 비교하여 크기 순서에 맞지 않으면 서로 교환하는 과정을 반복하여 정렬하는 방법",
    defShort: "인접한 두 원소를 비교·교환하며 반복해 정렬하는 방법",
    keywords: ["인접 원소 비교", "교환(Swap)", "O(n²)"],
    tables: [
      {
        caption: "동작 원리",
        headers: ["구분", "설명"],
        rows: [
          ["비교 대상", "인접한 두 원소를 앞에서부터 차례로 비교"],
          ["교환 조건", "앞의 값이 뒤의 값보다 크면 서로 교환(오름차순 기준)"],
          ["1회전 결과", "가장 큰 값이 맨 뒤로 이동(거품이 떠오르듯 확정)"],
          ["반복", "정렬되지 않은 구간을 하나씩 줄여가며 n−1회전 반복"],
          ["시간복잡도", "최선·평균·최악 모두 O(n²) — 구현은 가장 단순하나 비효율적"],
        ],
      },
    ],
    notes: ["교재 빅오 표기법 슬라이드에서 O(N²) 제곱형(주요 처리 loop 구조가 2중인 경우)의 대표 사례로 제시됨"],
  },
  {
