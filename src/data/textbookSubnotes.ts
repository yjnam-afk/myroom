...
export const SUBNOTES: TextbookSubnote[] = [
  ...
  },
  {
    ...
    defShort: "HW·응용 연계와 자원·프로세스·네트워크 관리를 맡는 프로그램",
    lead:
      "운영체제의 핵심 관리자, 커널",
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["유형", "원인", "설명"],
        rows: [
          ["내부 단편화", "할당 영역과의 크기 차이", "분할된 영역 > 프로그램 크기"],
          ["내부 단편화", "고정 분할 기법 사용", "프로그램 크기와 상관없이 분할·할당해 낭비 영역"],
          ["외부 단편화", "불연속 할당", "빈 공간 합은 충분하나 위치가 불연속이라 할당 불가"],
          ["외부 단편화", "가변 분할 기법 사용", "필요한 크기만큼 가변 분할해 불연속 낭비 공간 발생"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ["CXL.io", "PCIe 기능 제공", "장치 탐색·DMA 등 PCIe 유사 기능 / PCIe보다 낮은 접근 지연 / 모든 장치 유형 공통"],
          ["CXL.cache", "호스트·주변 캐시 일관성 유지", "비대칭형 — 호스트가 일관성 책임, 훼손 시 snoop 트랜잭션 / MESI 사용"],
          ["CXL.mem", "메모리 접근", "호스트가 캐시 라인(64byte) 단위 load/store로 주변장치 메모리를 로컬처럼 사용"],
        ],
      },
      {
        ...
        rows: [
          ["Type 1", "CXL.io + CXL.cache", "coherent 캐시만 있고 호스트 관리 메모리 없음 — NIC, 메모리 없는 가속기"],
          ["Type 2", "CXL.io + CXL.cache + CXL.mem", "coherent 캐시와 호스트 관리 메모리 모두 보유 — HBM 내장 GPU, FPGA 보드"],
          ["Type 3", "CXL.io + CXL.mem", "호스트가 관리하는 메모리로 구성 — DRAM·비휘발성 메모리 확장장치"],
        ],
      },
    ],
    notes: [
    ...
    tables: [
      {
        ...
        rows: [
          ...
          ["일관성 유지 기법", "VI(Valid & Invalid) 프로토콜", "MSI·MESI·MOSI·MOESI 프로토콜"],
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["원인", "설명"],
        rows: [
          ["멀티 프로세서 환경", "프로세서별 로컬 캐시에 같은 데이터가 복사됨"],
          ["변경 가능한 데이터의 공유", "한 캐시의 쓰기가 다른 캐시에 반영되지 않음"],
          ["입출력 동작(I/O Activity)", "I/O가 메모리를 직접 바꿔 캐시와 어긋남"],
        ],
      },
    ],
    notes: [
    ...
    tables: [
      {
        ...
        rows: [
          ...
          [
            "RAID 3",
            "바이트(Byte) 단위 스트라이핑 / 에러 처리를 위해 패리티 디스크 1개 사용",
          ],
          [
            "RAID 4",
            "블록(Block) 단위 스트라이핑 / 패리티 코드를 동일한 디스크에 저장",
          ],
          [
            "RAID 5",
          ...
          [
            "RAID 6",
            "패리티를 2개 디스크에 매번 다른 디스크로 저장(Double Parity) / 블록 단위 스트라이핑 / RAID 5보다 성능·용량을 줄이고 안정성 확보",
          ],
          [
            "RAID 10",
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["핵심 구조", "Systolic Array(시스톨릭 배열)", "SIMT(Single Instruction, Multiple Threads)"],
          ["작동 원리", "수만 개 연산 유닛이 격자로 서로 얽혀 연결", "수천 개 코어가 병렬 작동, 코어 그룹은 독립"],
          ["칩 연결", "ICI — 토러스 기반, 칩끼리 상하좌우(3D) 직접 연결", "NVLink·NVSwitch — 중앙 스위치 기반 1:1 초고속 통신"],
          ["약점", "딥러닝 이외 작업 불가, 유연성 낮음", "전력 소모 높음, 연산 코어 비율 상대적 낮음"],
        ],
      },
      {
        ...
        rows: [
          ["범용성", "그래픽·AI·과학연산 등 범용 활용", "전문성", "딥러닝의 대규모 행렬 연산에만 특화"],
          ["병렬성", "수천 개 코어로 데이터를 동시 처리", "효율성", "불필요한 제어를 줄여 전력·공간 절약"],
          ["접근성", "누구나 구매해 다양한 환경에 구축", "최적화", "텐서플로우 환경에서 처리 속도가 높음"],
        ],
      },
      {
        ...
        rows: [
          ["시간차 협업", "TPU 학습 / GPU 서비스", "학습은 TPU Pod로 빠르게, 추론은 GPU 서버로 배포"],
          ["전처리 구조", "GPU 데이터 가공 / TPU 학습", "이미지 회전·비디오 디코딩은 GPU, 행렬 연산 학습은 TPU"],
          ["엣지 구조", "TPU edge / GPU 심화 분석", "Edge TPU가 저전력 단순 감지, 상세 분석은 GPU"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["특징", "설명"],
        rows: [
          ["예측 불가", "프로세스 경쟁에 의한 출력값 예측 불가"],
          ["비균등", "프로세스 선점·자원 사용 방식의 일관성 부재"],
          ["분석 난해", "버그 발생 시 원인 파악 위한 분석 과정 난해"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["구분", "발생 조건", "설명"],
        rows: [
          ["우선순위 측면", "우선순위 기반 스케줄링", "높은 우선순위 프로세스가 끊임없이 생성되면 낮은 것은 기회 없음"],
          ["스케줄링 알고리즘 측면", "비선점형 스케줄링", "FCFS에서 긴 프로세스가 먼저 오면 뒤의 짧은 프로세스가 오래 대기"],
          ["자원 할당 측면", "자원 할당의 불공평성", "특정 자원을 반복 사용하는 프로세스가 계속 점유"],
          ["자원 관리 측면", "부적절한 자원 관리", "우선순위·사용 정책이 잘못되면 일부가 자원을 계속 점유"],
        ],
      },
      {
        ...
        headers: ["구분", "방지 기법", "설명"],
        rows: [
          ["우선순위", "우선순위 승격(Priority Aging)", "대기 시간에 따라 우선순위를 점진적으로 높임"],
          ["스케줄링", "타임 퀀텀 설정", "선점형의 타임 퀀텀 크기를 조정해 공정하게 CPU 할당"],
          ["스케줄링", "스케줄링 알고리즘 조합", "여러 알고리즘을 혼합해 장단점 상쇄"],
          ["자원 관리", "자원 할당 그래프", "할당 상태를 그래프로 관리해 지속 점유 방지"],
          ["자원 관리", "기한 기반 스케줄링", "프로세스마다 기한을 두고 기한 기준으로 스케줄링"],
          ["가중치", "지분 비례 CPU 배분", "낮은 우선순위 작업도 비례적으로 진전 확보"],
        ],
      },
      {
        ...
        rows: [
          ["원인", "Coffman 4대 조건 동시 만족", "선점형·우선순위 스케줄링(SJF·Priority), 자원 독점"],
          ["대상 범위", "둘 이상 프로세스 간 집단적 상호 의존", "단일 또는 일부 저우선순위 프로세스"],
          ["프로세스 상태", "Waiting/Blocked — 이벤트 발생 불가로 상태 변화 불가", "Ready/Waiting — 실행 가능하나 자원 할당만 거부"],
          ["자원 상태", "프로세스들이 자원을 나누어 쥔 채 고립", "자원은 계속 다른 프로세스에 할당·해제"],
          ["해결 메커니즘", "예방·회피·탐지 및 복구", "에이징·공정 큐잉·HRRN 스케줄링"],
          ["대표 알고리즘", "은행원 알고리즘, 자원할당그래프(RAG)", "HRRN, Priority with Aging"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ["1", "현재 프로세스", "–"],
          ["2", "인터럽트", "현재 상태를 PCB에 저장"],
          ["3", "프로세스 스케줄러", "다음 프로세스를 준비 큐에서 선택"],
          ["4", "디스패치", "다음 프로세스를 PCB에서 복구"],
          ["5", "다음 프로세스", "–"],
        ],
      },
      {
        ...
        headers: ["해결 방법", "설명"],
        rows: [
          ["문맥교환 발생 빈도 최소화", "다중 프로그래밍 수준을 낮추거나 스케줄러 알고리즘 개선"],
          ["쓰레드 사용", "경량 프로세스인 쓰레드로 전환되는 정보의 양을 최소화"],
        ],
      },
    ],
    notes: [
    ...
    tables: [
      {
        ...
        headers: ["유형", "키워드", "설명"],
        rows: [
          ["시스템 문맥", "PCB", "커널이 프로세스마다 두는 관리 정보 — PID·상태·스케줄링·계정"],
          ["메모리 문맥", "주소 공간", "코드·데이터·스택·힙과 스왑 영역 등 할당된 메모리"],
          ["하드웨어 문맥", "레지스터·PC", "어디까지 실행했는지 담는 CPU 레지스터와 프로그램 카운터"],
        ],
      },
      {
        caption: "문맥이 오가는 시점",
        headers: ["시점", "동작", "설명"],
        rows: [
          ["인터럽트·시스템 콜", "저장", "실행 중이던 프로세스의 문맥을 PCB에 기록"],
          ["디스패치", "복원", "다음 프로세스의 문맥을 PCB에서 CPU로 되돌림"],
          ["잦은 상태 전이", "오버헤드", "저장·복원이 반복돼 CPU 시간이 실행 아닌 교환에 쓰임"],
        ],
      },
    ],
    notes: [
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          [
            "재귀적 우선순위 상속",
            "프로세스마다 재귀적 자료구조를 두어 여러 형태의 역전을 해결하는 프로토콜",
          ],
          [
            "우선순위 올림",
      ...
      },
      {
        ...
        headers: ["발생 조건", "설명"],
        rows: [
          ["세마포어(Semaphore)", "임계영역(Critical Section)을 통해 특정 자원에 대한 점유"],
          ["선점 스케줄링", "우선순위 높은 Task가 낮은 Task의 CPU 점유를 선점"],
        ],
      },
      {
        ...
        rows: [
          ["기본적인 우선순위 역전", "해결 가능", "해결 가능", "해결 가능"],
          ["자원을 동시에 여러 개 소유", "해결 불가", "해결 가능", "해결 가능"],
          ["재귀적 형태로 자원 소유·요청", "해결 불가", "해결 불가", "해결 가능"],
  ...
  },
  {
    ...
    defShort: "응용이 커널 기능을 쓰려고 운영체제에 요청하는 정해진 호출 인터페이스",
    ...
    tables: [
      {
        ...
        headers: ["단계", "키워드", "설명"],
        rows: [
          ["① 호출", "라이브러리", "응용이 read() 같은 함수를 부르면 라이브러리가 syscall 명령 실행"],
          ["② 진입", "모드 비트", "CPU가 모드 비트를 바꿔 커널 모드로 들어감"],
          ["③ 처리", "호출 번호", "커널이 시스템 콜 번호로 해당 함수를 찾아 실행"],
          ["④ 복사", "유저 공간", "필요하면 결과 데이터를 유저 공간으로 복사"],
          ["⑤ 복귀", "반환 값", "반환 값을 두고 유저 모드로 돌아가 다음 명령 실행"],
        ],
      },
      {
        caption: "시스템 콜의 유형",
        headers: ["유형", "키워드", "설명"],
        rows: [
          ["프로세스 제어", "end·abort·execute", "프로그램의 실행·중지·종료를 커널에 맡김"],
          ["파일 조작", "create·open·close", "파일의 생성·삭제·열기·읽기·쓰기"],
          ["장치 관리", "request device", "장치 요구와 장치 속성의 조회·설정"],
          ["정보 유지", "시간·날짜", "프로세스·파일 속성과 시스템 정보 조회"],
          ["통신", "메시지·공유 메모리", "메시지 송수신과 상태 정보 전달"],
          ["보호", "set·get permission", "자원 접근 권한의 조회·설정"],
        ],
      },
      {
        ...
        rows: [
          ["권한", "모든 명령어·자원 접근", "자기 메모리만, HW 직접 접근 불가"],
          ["실행 주체", "OS 커널·드라이버·시스템 콜 핸들러", "일반 앱·브라우저·편집기"],
          ["오류 영향", "시스템 전체 장애(커널 패닉)", "해당 프로세스만 종료"],
          ["진입 계기", "인터럽트·시스템 콜·예외", "시스템 콜 종료·문맥교환"],
          ["보호", "스스로 책임", "커널 보호를 위한 제약"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["구분", "페이징", "세그멘테이션"],
        rows: [
          ["주소 구성", "p 페이지 번호 + d 오프셋(VPN·PPN)", "s 세그먼트 번호 + d 오프셋"],
          ["테이블 항목", "VPN→PPN 매핑, 접근권한·존재 비트(제어부분)", "세그먼트 테이블 — Base 주소·Limit(크기)"],
          ["동작 ①", "논리 주소에서 page number 확보", "논리 주소에서 segment number 확보"],
          ["동작 ②", "page table에서 frame number 확보", "segment table에서 base·limit 확보"],
          ["동작 ③", "frame number + offset으로 물리 주소 확인", "base + offset으로 물리 주소 확인(limit 검사)"],
        ],
      },
    ],
    notes: [
    ...
    tables: [
      {
        ...
        rows: [
          ...
          [
            ...
            "앞으로 가장 오랫동안 사용하지 않을 페이지를 교체하는 기법 — 부재 횟수 최소",
            "호출 순서·참조 상황을 미리 예측해야 해 실현 가능성 희박",
          ],
          [
      ...
      },
      {
        ...
        headers: ["구분", "사용 이유", "설명"],
        rows: [
          ["Size", "프로그램 크기 제약 탈피", "사용자 프로그램이 물리 메모리보다 커도 실행 가능"],
          ["Size", "메모리 크기 제약 탈피", "부족한 메인 메모리 사용 공간을 확보해서 사용"],
          ["Resource", "페이지 부재 대응", "부재 시 어떤 페이지 프레임을 선택·교체할지 결정"],
          ["Resource", "다중 프로세스 관리 용이", "다중 프로세스 생성을 효율적으로 처리 지원"],
          ["Resource", "성능 향상", "프로그램 적재에 필요한 입·출력 횟수 감소"],
        ],
      },
      {
        ...
        headers: ["구분", "주요 내용", "설명"],
        rows: [
          ["문제점", "Demand Paging", "필요 시에만 페이지 적재 → 더 많은 프로세스가 적재됨"],
          ["문제점", "Page Fault", "적재되지 않은 page 사용 시 부재 발생, OS가 요구"],
          ["문제점", "Thrashing", "page I/O가 빈번해져 성능 저하 초래"],
          ["해결방안", "Load Control", "새 프로세스 생성을 지연시켜 suspend Queue 대기"],
          ["해결방안", "Locality 활용", "시간·공간 지역성 집중 참조"],
          ["해결방안", "Working Set", "일정 시간 참조되는 page 집합을 주기억장치에 유지"],
          ["해결방안", "PFF", "page fault 빈도에 따라 residence set 크기 조정"],
        ],
      },
    ],
    notes: [
    ...
    tables: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["최적화 원칙 설계", "Load Control", "새 프로세스 생성을 지연시켜 suspend Queue에 대기시켜 Page Fault 감소"],
          [
            "최적화 원칙 설계",
            "PFF",
    ...
    tables: [
      {
        ...
        headers: ["구분", "발생원인", "설명"],
        rows: [
          ["① 리소스 부족", "CPU 성능 부족", "저사양 CPU로 필요한 자원량보다 부족"],
          ["① 리소스 부족", "저용량 메모리", "메모리 부족으로 리소스 추가 적용 불가"],
          ["② 부적절한 교체 정책", "요구 기반 페이지", "요구가 있을 때만 교체해 Locality 미고려"],
          ["② 부적절한 교체 정책", "페이지 교체 문제", "페이지 교체의 빈번한 시간 지연 발생"],
          ["③ 과도한 다중 프로그래밍", "다중 프로세스", "다중 프로세스 설계·의존성 문제"],
          ["③ 과도한 다중 프로그래밍", "할당 프레임 감소", "멀티 프로그램 과도로 할당 프레임 제한"],
        ],
      },
      {
        caption: "스레싱 발견 방법",
      ...
      },
      {
        ...
        rows: [
          ["Reference Page", "참조되는 페이지", "실행 위해 메모리에서 참조되는 페이지 / 참조 페이지가 많을수록 스레싱에 효과"],
          ["Working Set Size", "처리 시간 따라 변화", "프로세스 처리 시간에 따라 크기 변화 / Swap in·out을 적절히 써 워킹셋 충족"],
          ["Working Set Window", "Locality Set 수용 결정", "메모리상 참조하는 고정된 페이지 수 / 워킹셋 탐지 위해 Window size 최적화"],
        ],
      },
      {
        ...
        rows: [
          ["운영체제 설정", "Page Size 결정", "페이지 교체 오버헤드 방지 / Page 단위·TLB 범위 고려"],
          ["운영체제 설정", "역 페이지 테이블", "페이지 테이블 위한 메모리 공간 축소"],
          ["프로그램 설계", "Locality 고려", "Loop·Array 등 개발 언어 특성과 method 호출 구조 고려"],
          ["프로그램 설계", "페이지 Lock", "페이지 Locking으로 교체 방지 / 사용자 공간으로 I/O 차단"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["필요성", "설명"],
        rows: [
          ["Cache Access 시간 최소화", "집중 참조 부분을 캐시에 두어 접근 시간 단축"],
          ["Cache 적중률 향상", "지역성이 높을수록 적중률 극대화 달성"],
          ["Thrashing 최소화", "참조 집합을 유지해 페이지 교체 과다 방지"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["특징", "설명"],
        rows: [
          ["프로세스 상태 전이", "스케줄러에 의해 각 프로세스의 상태가 전이됨"],
          ["시스템 성능 최대화", "운영체제가 시스템 성능을 최대화하도록 수행"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["전이", "설명"],
        rows: [
          ["Dispatch", "준비 리스트의 우선순위 높은 프로세스가 CPU를 점유"],
          ["Timeout", "할당된 CPU Time Slice 초과로 CPU 점유 프로세스 변경"],
          ["Block I/O", "긴급 입출력 등 외부 요인으로 CPU 자원 반납"],
          ["Wake up", "입출력 작업 종료 등 기다리던 이벤트 완료"],
          ["Swap in", "프로세스가 CPU에 적재되는 이벤트"],
          ["Swap out", "프로세스가 CPU에서 해제되는 이벤트"],
        ],
      },
      {
        ...
        rows: [
          ["① Dispatch", "Ready → Running", "대기 프로세스가 CPU를 받아 실행 상태로 전이"],
          ["② Time Slice", "Running → Ready", "할당 시간 종료로 실행 중 프로세스가 준비 상태로"],
          ["③ 입출력", "Running → Waiting", "I/O 이벤트로 실행에서 대기 상태로 전이"],
          ["④ 시스템 콜", "Running → Waiting", "System call 이벤트로 대기 상태로 전이"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["해결방안", "설명"],
        rows: [
          ["교착상태 진입 방지 프로토콜", "교착상태 예방 기법과 회피 기법을 사용"],
          ["교착상태 허용 후 복구", "발생 여부를 탐지한 후 발생 시 회복 알고리즘 수행"],
          ["교착상태 미발생 가정", "문제를 무시하고 없는 것으로 가정, 발생 시 시스템 인위적 재시작"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["순위", "인터럽트", "설명"],
        rows: [
          ["1", "전원 이상", "예기치 못한 정전·Power 모듈 이상"],
          ["2", "기계 착오", "CPU의 기능적 오류"],
          ["3", "외부 신호", "타이머 만료·인터럽트 키(Ctrl+Alt+Del)·외부장치 요청"],
          ["4", "입출력", "I/O 장치의 전송 요구·전송 종료·데이터 이상"],
          ["5", "명령어 오류", "잘못된 명령어 수행"],
          ["6", "프로그램 검사", "오버플로우·언더플로우, 예외 처리(Exception)"],
          ["7", "SVC", "프로그램 처리 중 명령 요청으로 감시 프로그램 호출"],
        ],
      },
      {
        ...
        headers: ["구분", "설명"],
        rows: [
          ["인터럽트 벡터 테이블(IVT)", "인터럽트 발생 시 핸들러·ISR이 참조하는 서비스 루틴 주소 벡터 테이블"],
          ["인터럽트 서비스 루틴(ISR)", "IVT에 주소가 저장되어 처리 방식이 사전 정의된 기계어 코드 루틴"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        headers: ["구분", "정의"],
        rows: [
          ["PCB (Process Control Block)", "실행 상태와 자원 정보를 관리해 스케줄링·문맥교환의 기준이 되는 제어 블록"],
          ["TCB (Thread Control Block)", "스레드 실행 상태와 CPU 수행 정보를 관리해 경량 문맥교환 기준이 되는 제어 블록"],
        ],
      },
      {
        ...
        headers: ["구분", "PCB", "TCB"],
        rows: [
          ["관리 대상", "프로세스 — CPU를 간접적으로 할당", "스레드 — CPU 직접 할당 대상"],
          ["상태 정보", "New/Ready/Running/Waiting — 상태 기반 판단", "Ready/Running/Waiting — 스레드 단위 스케줄링"],
          ["실행 정보", "PC·레지스터·주소 공간 — Context Switch 시 전체 교체", "PC·SP·레지스터 — 경량 Context Switch"],
          ["스케줄링 정보", "우선순위·CPU 사용 시간 — 정책 판단 근거", "우선순위·타임 슬라이스 — 스케줄러 선택 기준"],
          ["역할 요약", "자원 소유·관리 — 스케줄링 기준 제공", "실행 흐름 관리 — CPU 스케줄링 실체"],
        ],
      },
      {
        caption: "PCB와 TCB 비교",
        headers: ["구분", "PCB", "TCB"],
        rows: [
          ["스케줄링 실체", "논리적 스케줄링 기준 단위", "실제 CPU 실행 단위"],
          ["주소 공간 관계", "독립 주소 공간 보유", "PCB의 주소 공간 공유"],
          ["병렬성 관점", "프로세스 간 병렬 실행", "동일 프로세스 내 병렬 실행"],
          ["스케줄링 효율", "전환 비용 큼", "전환 비용 작아 고효율"],
        ],
      },
    ],
    notes: [
  ...
  },
  {
    ...
    defShort: "응용을 작은 마이크로 서비스로 나눠 변경과 조합이 가능하게 한 아키텍처",
    lead:
      "서비스 단위의 분할 구조, MSA",
  ...
  },
  {
    ...
    defShort: "개발·운영 융합·협업의 개발 주기에 보안 주기를 포함하는 개발 방법론",
    lead:
  ...
  },
  {
    ...
    defShort: "AI 주도 계획·코딩을 사람이 검증하고 명세로 산출물을 얻는 방법론",
    ...
    defPair: [
      {
        ...
        lead: "AI 주도·사람 검증 개발",
        def: "AI 중심 생명주기로 계획·코딩은 AI가, 검증은 사람이 맡는 방법론",
        features: ["AI 역량 정렬", "단계 최소화", "고속 반복"],
      },
      {
        name: "SDD(Spec Driven Development)",
        lead: "명세 중심 자동 파생 개발",
        def: "기계가 읽는 사양에서 AI가 코드·테스트·문서를 파생하는 방법론",
