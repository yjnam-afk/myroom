    ...
    tables: [
      {
        ...
        headers: ["역할", "키워드", "설명"],
        rows: [
          ["하드웨어 제어", "물리 자원 직접 제어", "CPU·메모리·디스크 등 물리적 자원을 직접 제어하고 관리"],
          ["프로세스 관리", "CPU 시간 배분·스케줄링", "실행 중인 프로세스에 CPU 사용 시간을 배분하고 스케줄링"],
          ["메모리 관리", "메모리 할당·회수", "프로그램이 원활히 실행되도록 필요한 메모리를 할당하고 회수"],
          ["입출력(I/O) 관리", "주변 장치 데이터 교환", "키보드·마우스·네트워크 등 주변 장치와 데이터를 주고받도록 지원"],
          ["시스템 콜(System Call)", "안전한 자원 접근 창구", "응용 프로그램이 하드웨어 자원에 안전하게 접근하는 인터페이스 제공"],
        ],
      },
      {
