...
/**
 * 교재 세트는 scripts/build-textbook-map.ts 가 교재 서브노트 표에서 매 빌드마다
 * 뽑는다(textbookMap.json). 손으로 옮기면 내 지식이 섞이므로 손대지 않는다.
 * 교재 세트가 먼저 오고, 같은 제목의 옛 세트는 교재 것으로 대체된다.
 */
import textbookMap from "@/data/textbookMap.json";

const normTitle = (t: string) =>
  t.trim().toLowerCase().replace(/[(（][^)）]*[)）]/g, "").replace(/[\s()·,\-_/'’]/g, "");

const TEXTBOOK: CompareSet[] = (textbookMap as { compareSets: CompareSet[] }).compareSets;
const TB_TITLES = new Set(TEXTBOOK.map((x) => normTitle(x.title)));

export const compareSets: CompareSet[] = [
  ...TEXTBOOK,
  ...MANUAL.filter((x) => !TB_TITLES.has(normTitle(x.title))).map((x) => ({
    ...x,
    source: x.source ?? ("보조" as const),
  })),
];
