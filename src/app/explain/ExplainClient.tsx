...
export default function ExplainClient({ data }: { data: ExplainTopicData | null }) {
  ...
  return (
    <div>
      <PageHeader
        title="💡 토픽 설명"
        desc="여기는 이해 전용입니다 — 교재 슬라이드 + 답안지 템플릿 + 용어 매핑·옆 토픽·답안 한 줄 + 내 도식 + 남이 쓴 답안. 암기는 🥷암기 훈련장에서."
      />
      ...
      <div className="mt-6">
                          ...
                          <tr key={ri}>
                            {r.map((c, ci) => (
                              <td
                                key={ci}
                                // 2·3열은 데이터에 적힌 줄바꿈(\n) 그대로 — 한 줄 5~7자, 두 줄이면
                                // 2열·3열이 같은 줄 수로 나란히 서야 줄끼리 대응이 보인다. 폭에 따라 접지 않는다.
        ...
        {/* 학습 카드 — 용어 매핑·옆 토픽·답안 한 줄만. 줄글 설명 절은 전부 뺐다. 교재 다음에 온다. */}
